...
async function checked(args, input) {
  ...
  if (result.code !== 0)
    throw new Error(`${args.slice(0, 3).join(" ")} failed: ${result.stderr}`);
...
const schemaQuery = await readFile(
  path.join(root, "supabase/schema.sql"),
  "utf8",
);
const migrationQueries = await Promise.all(
  (await readdir(path.join(root, "supabase/migrations")))
    .filter((f) => f.endsWith(".sql"))
    .sort()
    .map((file) =>
      readFile(path.join(root, "supabase/migrations", file), "utf8"),
    ),
);
const testFiles = [
  "personal_reimbursement.sql",
  "automatic_reimbursement_period.sql",
  "unified_monthly_budget.sql",
  "unified_budget_partial_reservation.sql",
  "fund_workflow.sql",
  ...
  "quick_expense_receipt_ocr.sql",
  "finance_task_sources.sql",
  "advance_settlement_drafts.sql",
  "advance_settlement_lifecycle.sql",
...
if (requestedTests.some((file) => !testFiles.includes(file)))
  throw new Error(
    "Pass only named SQL test files from the workflow test list.",
  );
const selectedTests = requestedTests.length ? requestedTests : testFiles;
const testQueries = await Promise.all(
  selectedTests.map(async (file) => ({
    file,
    query: await readFile(path.join(root, "supabase/tests", file), "utf8"),
  })),
);
try {
  await checked([
...
insert into finance.bank_transactions(id,organization_id,bank_account_id,transacted_at,description,withdrawal_amount,deposit_amount) values('${bank}','${org}','${acct}',now()-interval '1 second','Test withdrawal',1000,0);
...
`);
  ...
  const allocation = JSON.stringify({
    ...
    items: [{ transaction_id: ids.tx, purpose: "DISBURSEMENT", amount: 60 }],
  });
  const commands = [1, 2].map(
    (i) =>
      `begin; set role service_role; select finance.workflow_command('${org}','${actor}','PAYMENT_ALLOCATE','${allocation}','race-${i}'); select pg_sleep(0.5); commit;`,
  );
  ...
  assert.equal(
    races.filter((r) => r.code === 0).length,
    1,
    `one concurrent allocation must succeed: ${JSON.stringify(races)}`,
  );
  assert(
    races.find((r) => r.code !== 0)?.stderr.includes("미지급액을 초과"),
    "other allocation fails due to refreshed balance",
  );
  assert.equal(
    (
      await sql(
        `select sum(amount) from finance.workflow_allocations where transaction_id='${ids.tx}';`,
      )
    ).trim(),
    "60",
  );
  console.log(
    "PASS: concurrent database sessions serialize over-allocation (60 of 100)",
  );
  ...
  await sql(
    `select finance.workflow_command('${org}','${actor}','ALLOCATION_REVERSE',jsonb_build_object('id',(select id from finance.workflow_allocations where transaction_id='${ids.tx}'),'reason','Test correction'),'reverse-race');`,
  );
  ...
  assert.equal(
    (
      await sql(
        `select count(*) from finance.workflow_operations where organization_id='${org}' and operation_key='race-duplicate';`,
      )
    ).trim(),
    "1",
  );
  assert.equal(
    (
      await sql(
        `select count(*) from finance.workflow_allocations where transaction_id='${ids.tx}';`,
      )
    ).trim(),
    "2",
  );
  console.log(
    "PASS: concurrent identical operation persisted once and reloaded",
  );

  const trustSource = randomUUID(),
    trustContract = randomUUID();
...
`);
  ...
  const trustInputs = trustRows.map((row) =>
    JSON.stringify({
      id: row.id,
      lock_version: 1,
  ...
  const trustCommands = trustInputs.map(
    (input, index) =>
      `begin; set role service_role; select finance.trust_command('${org}','${actor}','REQUEST_SUBMIT','${input}','trust-race-${index}'); select pg_sleep(0.5); commit;`,
  );
  const trustRaces = await Promise.all(
    trustCommands.map((query) =>
      run(
        [
          "exec",
          "-i",
  ...
  assert.equal(
    trustRaces.filter((result) => result.code === 0).length,
    1,
    JSON.stringify(trustRaces),
  );
  assert(
    trustRaces
      .find((result) => result.code !== 0)
      ?.stderr.includes("요청 가능액을 초과"),
    "second request must see the first committed reservation",
  );
  ...
  assert.equal(
    (
      await sql(
        `select count(*) from finance.workflow_submissions where organization_id='${org}';`,
      )
    ).trim(),
    "1",
  );
  console.log(
    "PASS: concurrent trust requests cannot reserve 1200 against 1000; same-key retry retains one submission",
  );
  const accountingSignature = (
    await sql(
      `select finance.accounting_source('${org}','RECOGNITION','${ids.tx}')->>'signature';`,
    )
  ).trim();
  const accountingInput = JSON.stringify({
    source_kind: "RECOGNITION",
    source_id: ids.tx,
    source_signature: accountingSignature,
    voucher_date: "2026-09-08",
    memo: "Concurrent draft",
    lines: [],
  });
  const accountingCommands = [1, 2].map(
    (i) =>
      `begin; set role service_role; select finance.accounting_command('${org}','${actor}','DRAFT_CREATE','${accountingInput}','accounting-race-${i}'); select pg_sleep(0.5); commit;`,
  );
  const accountingRaces = await Promise.all(
    accountingCommands.map((query) =>
      run(
        [
          "exec",
          "-i",
  ...
  assert.equal(
    accountingRaces.filter((result) => result.code === 0).length,
    1,
    JSON.stringify(accountingRaces),
  );
  assert(
    accountingRaces
      .find((result) => result.code !== 0)
      ?.stderr.includes("이미 연결된 전표"),
    "second accounting editor must see the committed source link",
  );
  await sql(
    accountingCommands[
      accountingRaces.findIndex((result) => result.code === 0)
    ],
  );
  assert.equal(
    (
      await sql(
        `select count(*) from finance.workflow_voucher_links where organization_id='${org}' and source_id='${ids.tx}';`,
      )
    ).trim(),
    "1",
  );
  console.log(
    "PASS: concurrent accounting draft creation produces one source voucher; same-key retry preserves it",
  );
  const advanceUse = randomUUID();
  await sql(
    `insert into finance.quick_expense_records(id,organization_id,source_type,payment_method,occurred_at,amount,counterparty,usage_description,budget_item,approval_skip_reason,direct_expense_decision,record_status,recorded_by_label) values('${advanceUse}','${org}','MANUAL','CASH','2026-03-15',50,'Shop','Concurrent use','Test','Reviewed','REQUIRED','NEEDS_RESOLUTION','Test');`,
  );
  const advanceInputs = [];
...
select jsonb_build_object('transaction_id',id,'source_signature',finance.advance_settlement_source('${org}',id)->>'signature','title','Concurrent settlement','funding',jsonb_build_array(jsonb_build_object('allocation_id','${allocationId}','kind','INITIAL')),'usage',jsonb_build_array(jsonb_build_object('source_kind','QUICK','source_id','${advanceUse}','signature',finance.advance_settlement_usage_source('${org}','QUICK','${advanceUse}')->>'signature','evidence_file_id',null))) from finance.workflow_transactions where organization_id='${org}' and source_id='${resolution}';`);
  ...
  const advanceCommands = advanceInputs.map(
    (input, index) =>
      `begin; set role service_role; select finance.advance_settlement_command('${org}','${actor}','DRAFT_SAVE','${input}','advance-race-${index}'); select pg_sleep(0.5); commit;`,
  );
  const advanceRaces = await Promise.all(
    advanceCommands.map((query) =>
      run(
        [
          "exec",
          "-i",
  ...
  assert.equal(
    advanceRaces.filter((result) => result.code === 0).length,
    1,
    JSON.stringify(advanceRaces),
  );
  assert(
    advanceRaces
      .find((result) => result.code !== 0)
      ?.stderr.includes("사용 원본이 정산"),
    "second draft must see committed usage reservation",
  );
  await sql(
    advanceCommands[advanceRaces.findIndex((result) => result.code === 0)],
  );
  assert.equal(
    (
      await sql(
        `select count(*) from finance.advance_settlement_drafts where organization_id='${org}';`,
      )
    ).trim(),
    "1",
  );
  assert.equal(
    (
      await sql(
        `select count(*) from finance.advance_settlement_claims where organization_id='${org}' and source_id='${advanceUse}';`,
      )
    ).trim(),
    "1",
  );
  console.log(
    "PASS: concurrent advance drafts reserve one original usage; failed draft rolls back and same-key retry reloads",
  );
  ...
  const approvalBefore = {
    ...
    approvalLine: [{ approver: "Test", role: "actor", status: "결재대기" }],
    currentApprover: "Test actor",
    history: [],
    expenseItems: [],
  };
  const approvalAfter = {
    ...
    approvalLine: [
      {
        ...
        role: "actor",
        status: "승인완료",
        processedAt: "2026-09-08T10:00:00+09:00",
      },
    ],
    currentApprover: undefined,
    approvedAt: "2026-09-08T10:00:00+09:00",
...
insert into finance.expense_authorization_bindings(resolution_id,organization_id,author_user_id,steps) values('${approvalId}','${org}','${actor}','${JSON.stringify([{ order: 1, approver_user_id: actor, legacy_step: { approver: "Test", role: "actor" } }])}');`);
  const approvalCommands = [1, 2].map(
    (index) =>
      `begin; set role service_role; select finance.legacy_expense_command('${org}','${actor}','APPROVAL','${approvalId}','${JSON.stringify(approvalBefore)}','${JSON.stringify({ command: "APPROVE", after: approvalAfter })}','approval-race-${index}'); select pg_sleep(0.5); commit;`,
  );
  const approvalRaces = await Promise.all(
    approvalCommands.map((query) =>
      run(
        [
          "exec",
          "-i",
  ...
  assert.equal(
    approvalRaces.filter((result) => result.code === 0).length,
    1,
    JSON.stringify(approvalRaces),
  );
  assert(
    approvalRaces
      .find((result) => result.code !== 0)
      ?.stderr.includes("원본이 변경"),
    "second approval must see committed original",
  );
  await sql(
    approvalCommands[approvalRaces.findIndex((result) => result.code === 0)],
  );
  assert.equal(
    (
      await sql(
        `select count(*) from finance.expense_workflow_audit_logs where resolution_id='${approvalId}' and action='AUTH:APPROVAL';`,
      )
    ).trim(),
    "1",
  );
  console.log(
    "PASS: concurrent UUID approvals commit one transition and audit; same-key retry preserves it",
  );
  ...
  const documentCreate = {
    document: {
      ...
      purpose: "Concurrent approval verification",
      amount: 0,
    },
    steps: [{ approverLabel: "Test", approverRole: "Actor" }],
    lines: [],
  };
  const createOutput = await sql(
    `select approval.document_command('${org}','${actor}','CREATE','${documentId}',0,'${JSON.stringify(documentCreate)}','doc-create');`,
  );
  const createdDoc = JSON.parse(createOutput.trim().split("\n").at(-1));
  const bindOutput = await sql(
    `select approval.document_command('${org}','${actor}','BIND','${documentId}',${createdDoc.version},'${JSON.stringify({ drafter_user_id: actor, steps: [{ order: 1, user_id: actor }], reason: "Explicit fixture identity" })}','doc-bind');`,
  );
  const boundDoc = JSON.parse(bindOutput.trim().split("\n").at(-1));
  const submittedOutput = await sql(
    `select approval.document_command('${org}','${actor}','SUBMIT','${documentId}',${boundDoc.version},'{}','doc-submit');`,
  );
  const submittedDoc = JSON.parse(submittedOutput.trim().split("\n").at(-1));
  const documentCommands = [1, 2].map(
    (index) =>
      `begin;set role service_role;select approval.document_command('${org}','${actor}','APPROVE','${documentId}',${submittedDoc.version},'{}','document-race-${index}');select pg_sleep(0.5);commit;`,
  );
  const documentRaces = await Promise.all(
    documentCommands.map((query) =>
      run(
        [
          "exec",
          "-i",
  ...
  assert.equal(
    documentRaces.filter((result) => result.code === 0).length,
    1,
    JSON.stringify(documentRaces),
  );
  assert(
    documentRaces
      .find((result) => result.code !== 0)
      ?.stderr.includes("기안이 변경"),
    "other document approval sees committed version",
  );
  await sql(
    documentCommands[documentRaces.findIndex((result) => result.code === 0)],
  );
  assert.equal(
    (
      await sql(
        `select count(*) from approval.audit_logs where document_id='${documentId}' and action_type='AUTH:APPROVE';`,
      )
    ).trim(),
    "1",
  );
  console.log(
    "PASS: concurrent document UUID approvals persist one decision and audit, with stable retries",
  );
  const denied = await run(
    ["exec", "-i", name, "psql", "-U", "postgres", "-v", "ON_ERROR_STOP=1"],
    `set role authenticated; select * from finance.workflow_payments;`,
  );
  assert.notEqual(
    denied.code,
    0,
    "authenticated direct privileged table read must fail",
  );
  await sql(
    `insert into storage.objects(bucket_id,name) values('finance-workflow','isolated-private'),('isolated-public','legacy-visible');`,
  );
  const storageRows = await sql(
    `set role authenticated; select count(*) from storage.objects where bucket_id='finance-workflow'; select count(*) from storage.objects where bucket_id='isolated-public';`,
  );
  assert.deepEqual(
    storageRows.trim().split("\n").slice(-2),
    ["0", "1"],
    "private workflow objects stay hidden even with a legacy broad Storage policy",
  );
  console.log("PASS: database role access denied");
} finally {
  if (created) {
    const actualLabel = await checked([
      "inspect",
      "--format",
      '{{index .Config.Labels "dbapt-erp.test"}}',
      name,
    ]);
    assert.equal(
      actualLabel.trim(),
      "finance-workflow",
      "only this runner owned container can be removed",
    );
