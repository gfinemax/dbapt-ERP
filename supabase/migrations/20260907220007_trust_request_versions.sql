-- Contract configuration, item review and immutable submission versions.
-- Drafts remain available before contract policy is verified.
alter table finance.workflow_contract_versions alter column management_account_id drop not null;
alter table finance.workflow_contract_versions add column lock_version integer not null default 1;
alter table finance.workflow_contract_versions add column previous_version_id uuid;
alter table finance.workflow_contract_versions add foreign key(organization_id,previous_version_id)
 references finance.workflow_contract_versions(organization_id,id);
alter table finance.workflow_trust_requests alter column contract_version_id drop not null;
alter table finance.workflow_trust_requests add column lock_version integer not null default 1;
alter table finance.workflow_files add column contract_version_id uuid;
alter table finance.workflow_files add column document_type text not null default '';
alter table finance.workflow_files add foreign key(organization_id,contract_version_id)
 references finance.workflow_contract_versions(organization_id,id);

insert into storage.buckets(id,name,public,file_size_limit,allowed_mime_types)
values('finance-workflow','finance-workflow',false,20971520,array['application/pdf','image/jpeg','image/png','image/webp'])
on conflict(id) do nothing;
-- Existing broad Storage policies must not expose this private workflow bucket.
create policy workflow_files_server_only on storage.objects as restrictive for all to anon,authenticated
using(bucket_id<>'finance-workflow') with check(bucket_id<>'finance-workflow');

create function finance.trust_contract_guard() returns trigger language plpgsql security invoker set search_path='' as $$
begin
 if tg_op='DELETE' then raise exception '계약 버전은 삭제하지 않고 폐기 이력으로 보존합니다.'; end if;
 if old.status<>'DRAFT' and (new.status<>'RETIRED' or old.status='RETIRED' or
    (to_jsonb(new)-'status'-'lock_version') is distinct from (to_jsonb(old)-'status'-'lock_version')) then
  raise exception '확인된 계약은 새 버전으로 변경해주세요.';
 end if;
 return new;
end $$;
create trigger trust_contract_guard before update or delete on finance.workflow_contract_versions
for each row execute function finance.trust_contract_guard();

create function finance.trust_account_guard() returns trigger language plpgsql security invoker set search_path='' as $$
begin
 perform pg_advisory_xact_lock(hashtextextended(old.organization_id::text,739));
 if exists(select 1 from finance.workflow_contract_versions c where c.status<>'DRAFT' and
   (c.management_account_id=old.id or (c.conditions->'operating_account_ids') ? old.id::text)) then
  if tg_op='DELETE' or new.organization_id is distinct from old.organization_id or new.account_no is distinct from old.account_no or
     new.bank_name is distinct from old.bank_name or new.deleted_at is distinct from old.deleted_at then raise exception '확인된 계약의 계좌 원본은 보존합니다. 새 계좌와 계약 버전을 등록해주세요.'; end if;
 end if;
 if tg_op='DELETE' then return old; else return new; end if;
end $$;
create trigger trust_account_guard before update or delete on finance.bank_accounts for each row execute function finance.trust_account_guard();

create function finance.trust_status(p_org uuid,p_request uuid) returns text
language sql stable security invoker set search_path='' as $$
 select case
 when count(*)=0 then 'DRAFT'
 when bool_and(status='WITHDRAWN') then 'WITHDRAWN'
 when bool_or(status='WITHDRAWAL_PENDING') then 'WITHDRAWAL_PENDING'
 when bool_or(status='SUPPLEMENT') then 'SUPPLEMENT'
 when bool_and(status in ('APPROVED','WITHDRAWN')) then 'APPROVED'
 when bool_or(status in ('APPROVED','PARTIAL')) then 'PARTIAL'
 when bool_or(status='REVIEWING') then 'REVIEWING'
 when bool_and(status in ('REJECTED','WITHDRAWN')) then 'REJECTED'
 else 'DRAFT' end
 from finance.workflow_trust_items where organization_id=p_org and request_id=p_request;
$$;

create function finance.trust_item_paid(p_org uuid,p_item uuid) returns numeric
language sql stable security invoker set search_path='' as $$
 select coalesce(sum(case when a.purpose='RETURN' then -a.amount else a.amount end),0)
 from finance.workflow_allocations a where a.organization_id=p_org and a.trust_item_id=p_item
 and not exists(select 1 from finance.workflow_allocation_reversals r where r.allocation_id=a.id);
$$;

-- Reopened review retains actual paid amounts and reserves only the unpaid part.
create or replace function finance.workflow_transaction_amounts(p_org uuid,p_tx uuid)
returns jsonb language sql stable security invoker set search_path='' as $$
 ...
 select coalesce(sum(case when i.status in ('REVIEWING','SUPPLEMENT') or (i.status='WITHDRAWAL_PENDING' and i.withdrawal_from_status in ('REVIEWING','SUPPLEMENT')) then greatest(0,i.requested_amount-finance.trust_item_paid(p_org,i.id)) else 0 end),0) pending,
 ...
 coalesce(sum(case when i.status in ('APPROVED','PARTIAL') or (i.status='WITHDRAWAL_PENDING' and i.withdrawal_from_status in ('APPROVED','PARTIAL')) then greatest(0,i.approved_amount-finance.trust_item_paid(p_org,i.id)) else 0 end),0) approved_unpaid
...
create function finance.trust_read(p_org uuid,p_actor uuid) returns jsonb
language plpgsql stable security invoker set search_path='' as $$
declare m finance.reimbursement_members;
begin
 m:=finance.workflow_actor(p_org,p_actor);
 if not m.permissions && array['ADMIN','APPROVE','PAY','CLOSE','SENIOR'] then raise exception '신탁 업무 조회 권한이 필요합니다.'; end if;
 return jsonb_build_object(
 'contracts',coalesce((select jsonb_agg(to_jsonb(c) order by created_at desc) from finance.workflow_contract_versions c where organization_id=p_org),'[]'),
 'requests',coalesce((select jsonb_agg(to_jsonb(q) order by created_at desc) from finance.workflow_trust_requests q where organization_id=p_org),'[]'),
 'items',coalesce((select jsonb_agg(to_jsonb(i)||jsonb_build_object('paid_amount',finance.trust_item_paid(p_org,i.id))) from finance.workflow_trust_items i where organization_id=p_org),'[]'),
 'files',coalesce((select jsonb_agg(to_jsonb(f)-'bucket'-'path' order by uploaded_at desc) from finance.workflow_files f where organization_id=p_org),'[]'),
 'submissions',coalesce((select jsonb_agg((to_jsonb(s)-'snapshot')||jsonb_build_object('snapshot',s.snapshot->'print') order by submitted_at desc) from finance.workflow_submissions s where organization_id=p_org),'[]'),
 'events',coalesce((select jsonb_agg((to_jsonb(e)-'before_data'-'after_data')||jsonb_build_object('before_data',e.before_data-'path'-'bucket','after_data',e.after_data-'path'-'bucket') order by created_at desc) from finance.workflow_events e where organization_id=p_org and action like 'TRUST:%'),'[]'),
 'accounts',coalesce((select jsonb_agg(jsonb_build_object('id',a.id,'label',concat_ws(' ',a.bank_name,a.account_name,'***'||right(a.account_no,4)))) from finance.bank_accounts a where organization_id=p_org and deleted_at is null),'[]'));
end $$;

create function finance.trust_command(p_org uuid,p_actor uuid,p_command text,p_data jsonb,p_key text)
...
declare m finance.reimbursement_members; op finance.workflow_operations;
 ...
 f finance.workflow_files; item jsonb; entry jsonb; conditions jsonb; result jsonb; before_value jsonb;
...
begin
 ...
 if p_command='CONTRACT_SAVE' then
  conditions:=coalesce(p_data->'conditions','{}');
  if jsonb_typeof(conditions)<>'object' then raise exception '계약 조건 형식을 확인해주세요.'; end if;
  ...
  if target is null then
   ...
   values(p_org,coalesce(previous.contract_key,gen_random_uuid()),coalesce((select max(version)+1 from finance.workflow_contract_versions where organization_id=p_org and contract_key=previous.contract_key),1),
    trim(coalesce(p_data->>'name','')),trim(coalesce(p_data->>'trustee','')),trim(coalesce(p_data->>'reference','')),contract_id,'DRAFT',conditions,p_actor,previous.id) returning * into c;
  else
   ...
   update finance.workflow_contract_versions set name=trim(coalesce(p_data->>'name','')),trustee=trim(coalesce(p_data->>'trustee','')),reference=trim(coalesce(p_data->>'reference','')),
    management_account_id=contract_id,conditions=conditions,lock_version=lock_version+1 where id=c.id returning * into c;
  end if;
  ...
  before_value:=to_jsonb(c); conditions:=c.conditions;
  if p_command='CONTRACT_VERIFY' then
   ...
   if jsonb_typeof(conditions->'allowed_source_kinds') is distinct from 'array' or jsonb_array_length(conditions->'allowed_source_kinds')=0 or
      jsonb_typeof(conditions->'required_document_types') is distinct from 'array' or jsonb_typeof(conditions->'consent_roles') is distinct from 'array' or
      jsonb_typeof(conditions->'operating_allowed') is distinct from 'boolean' or jsonb_typeof(conditions->'operating_advance_allowed') is distinct from 'boolean' then raise exception '적용 거래·필수서류·공동 동의·운영비 조건을 명시적으로 확인해주세요.'; end if;
   if exists(select 1 from jsonb_array_elements_text(conditions->'allowed_source_kinds') v where v not in ('RESOLUTION','QUICK','PERSONAL','REFUND','ADVANCE')) then raise exception '계약 적용 거래 유형을 확인해주세요.'; end if;
   if conditions->>'no_limit' is distinct from 'true' and (coalesce((conditions->>'max_request_amount')::numeric,0)<=0 or (conditions->>'max_request_amount')::numeric<>trunc((conditions->>'max_request_amount')::numeric)) then raise exception '집행 한도 또는 한도 없음의 계약 근거를 확인해주세요.'; end if;
   if conditions->>'operating_allowed'='true' then
    if trim(coalesce(conditions->>'operating_basis',''))='' or jsonb_typeof(conditions->'operating_account_ids') is distinct from 'array' or jsonb_array_length(conditions->'operating_account_ids')=0 then raise exception '운영계좌 집행 근거와 계좌를 설정해주세요.'; end if;
    if exists(select 1 from jsonb_array_elements_text(conditions->'operating_account_ids') v where not exists(select 1 from finance.bank_accounts a where a.organization_id=p_org and a.id::text=v and a.deleted_at is null and a.id<>c.management_account_id)) then raise exception '조합 운영계좌를 확인해주세요.'; end if;
   end if;
   if conditions->>'operating_advance_allowed'='true' and (conditions->>'operating_allowed'<>'true' or trim(coalesce(conditions->>'advance_settlement_terms',''))='') then raise exception '운영비 선교부와 정산 조건을 확인해주세요.'; end if;
...
do $$ declare f record; begin
 for f in select p.oid::regprocedure signature from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='finance' and p.proname like 'trust_%' loop
