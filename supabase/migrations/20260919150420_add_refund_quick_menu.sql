alter table finance.user_quick_menu_preferences
  drop constraint user_quick_menu_preferences_menu_ids_check1,
  add constraint user_quick_menu_preferences_menu_ids_check1 check(menu_ids <@ array[
    'members','collections','refunds','bank','cards','resolution','payments','evidence','arrears','advance','expenses',
    'corporate-use','expense-entry','personal-refund','trust-operating','trust-business','readiness'
  ]::text[]);

create or replace function finance.quick_menu_preferences_save(p_org uuid,p_actor uuid,p_menu_ids text[],p_expected_revision integer,p_operation_key text)
...
begin
 ...
 if not (p_menu_ids <@ array['members','collections','refunds','bank','cards','resolution','payments','evidence','arrears','advance','expenses','corporate-use','expense-entry','personal-refund','trust-operating','trust-business','readiness']::text[]) then raise exception '지원하지 않는 퀵메뉴가 포함되어 있습니다.'; end if;
...
revoke all on function finance.quick_menu_preferences_save(uuid,uuid,text[],integer,text) from public,anon,authenticated;
grant execute on function finance.quick_menu_preferences_save(uuid,uuid,text[],integer,text) to service_role;
