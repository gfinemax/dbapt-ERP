...
declare
 ...
 account_id uuid:=gen_random_uuid(); deposit_id uuid:=gen_random_uuid(); deposit2_id uuid:=gen_random_uuid(); withdrawal_id uuid:=gen_random_uuid(); wrong_withdrawal_id uuid:=gen_random_uuid();
 saved jsonb; retried jsonb; allocation jsonb; second_allocation jsonb; refund jsonb; approved jsonb; paid jsonb; close_state jsonb; assessment uuid; allocation_id uuid; refund_id uuid;
begin
 ...
 if (select count(*) from finance.collection_ledger_events where organization_id=org)<>7 then raise exception 'TEST: audit events missing'; end if;
