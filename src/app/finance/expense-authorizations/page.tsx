...
export default async function Page() {
  ...
  try {
    ...
    return (
      <ReimbursementLogin
        title="작성자·결재선 계정 연결"
        description="관리자 계정으로 로그인해줘."
        error={
          error instanceof Error ? error.message : "권한 확인이 필요합니다."
        }
      />
    );
  }
  ...
  try {
    ...
    const { data, error } = await expenseDb()
      ...
      .select("user_id,display_name,permissions")
      .eq("organization_id", actor.organization_id)
      .eq("active", true)
      .order("display_name");
    if (error)
      throw new Error("같은 조합의 사용자 목록을 불러오지 못했습니다.");
    members = data ?? [];
  } catch (error) {
    message =
  ...
  const content = message ? (
    ...
    <ExpenseAuthorizationPage records={records ?? []} members={members} />
  );
  return (
    <ErpShell
      activeLabel="회계/자금"
      ...
      userLabel={actor.display_name}
    >
      <div className="mx-auto max-w-5xl p-5">{content}</div>
    </ErpShell>
  );
}
