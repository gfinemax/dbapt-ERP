...
export default async function Page({ searchParams }: { searchParams: Promise<{ kind?: string; connection?: string; q?: string; source_kind?: string; source_id?: string }> }) {
  ...
  const content = workspace ? <ExpenseWorkspacePage key={JSON.stringify(query)} workspace={workspace} expenseDetails={expenseDetails} initialKind={query.kind} initialConnection={query.connection} initialSearch={query.q} initialSourceKind={query.source_kind} initialSourceId={query.source_id} />
