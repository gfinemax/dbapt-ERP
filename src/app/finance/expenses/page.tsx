...
export default async function Page({ searchParams }: { searchParams: Promise<{ kind?: string; connection?: string; page?: string; q?: string; sort?: string; status?: string; source_kind?: string; source_id?: string }> }) {
  ...
  try { member = await reimbursementIdentity(); if (member) [workspace, expenseDetails] = await Promise.all([loadExpenseWorkspace(), listOperatingExpenseDetails(member.organization_id)]); } catch (error) { message = error instanceof Error ? error.message : "지출 원본을 불러오지 못했어."; }
