...
export default async function Page({ searchParams }: { searchParams: Promise<{ tab?: string; q?: string }> }) {
  ...
  return <ErpShell userLabel={member?.display_name ?? "로그인 필요"} logoutAction={reimbursementLogout} activeLabel="회계/자금" activeWorkspaceLabel="전표·증빙관리" activeDetailLabel={query.tab === "UNPAID" ? "조합 지급대기" : "전체 지급내역"}><div className="mx-auto max-w-7xl">{content}</div></ErpShell>;
}
