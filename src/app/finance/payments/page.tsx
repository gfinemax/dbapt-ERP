...
export default async function Page({ searchParams }: { searchParams: Promise<{ tab?: string; q?: string }> }) {
  ...
  let member; let workspace; let trust; let message;
  try {
  ...
  const content = workspace && trust ? <FundPaymentPage key={`${query.tab ?? "ALL"}:${query.q ?? ""}`} workspace={workspace} trust={trust} initialTab={query.tab} initialSearch={query.q} />
