...
export default async function ReportPage({searchParams}:{searchParams:Promise<{month?:string;revision?:string}>}) {
  ...
  return <main className="mx-auto max-w-6xl p-8 text-sm"><h1 className="mb-4 text-2xl font-bold">{r.month.slice(0,7)} 월 예산 마감 보고서 · 버전 {r.revision}</h1><p>확정 일시: {new Date(r.created_at).toLocaleString("ko-KR",{timeZone:"Asia/Seoul"})}</p><p className="my-3">사유: {r.reason}</p><p className="mb-5">간편지출·개인 정산 승인액·기안 집행 예약액 기준. 회계장부 전체 지출 보고서와 별도이며, 실제 지급액과 예산 사용액은 구분합니다.</p>
    <table className="w-full text-right"><thead><tr className="border-b"><th className="text-left">예산항목</th><th>월 예산</th><th>간편지출</th><th>개인 정산</th><th>사용액</th><th>예약액</th><th>가용액</th></tr></thead><tbody>{r.snapshot.budgets.map(b=><tr key={b.id} className="border-b"><th className="py-3 text-left font-normal">{b.budget_item}</th>{[b.monthly_amount,b.quick_amount,b.personal_amount,budgetUsed(b),b.reserved_amount,budgetRemaining(b)].map((n,i)=><td key={i}>{Number(n).toLocaleString("ko-KR")}</td>)}</tr>)}</tbody></table><p className="mt-4">단위: 원 · 이 보고서는 확정 시점의 보관본입니다.</p><p className="mt-3 print:hidden">브라우저 인쇄 메뉴(Ctrl+P)에서 인쇄하거나 PDF로 저장할 수 있어.</p></main>;
}
