...
import {
  ...
  createApprovalAttachmentDownloadUrl,
  createApprovalDocument,
  createContractFromApproval,
  createContractPaymentExpense,
  createExpenseDraftFromApproval,
  createMeetingAgenda,
...
export async function createApprovalAction(formData: FormData) {
  const lines = [1, 2, 3]
    .map((index) => ({
      ...
      budgetItem: text(formData, `lineBudgetItem${index}`),
      description: text(formData, `lineDescription${index}`),
      partnerName: text(formData, `linePartner${index}`),
      supplyAmount: Number(text(formData, `supplyAmount${index}`)) || 0,
      vatAmount: Number(text(formData, `vatAmount${index}`)) || 0,
    }))
    .filter(
      (line) =>
        line.partnerName ||
  ...
  const draft = {
    ...
    contractStartDate: text(formData, "contractStartDate"),
    contractEndDate: text(formData, "contractEndDate"),
    contractPaymentTerms: text(formData, "contractPaymentTerms"),
    paymentSchedule: [1, 2, 3]
      .map((index) => ({
        amount: Number(text(formData, `scheduleAmount${index}`)) || 0,
        dueDate: text(formData, `scheduleDate${index}`),
...
export async function createMeetingAgendaAction(formData: FormData) {
  ...
  await createMeetingAgenda(
    id,
    text(formData, "actorLabel"),
    text(formData, "meetingBody") as "BOARD" | "DELEGATES" | "GENERAL_ASSEMBLY",
  );
  revalidatePath(`/approval/${id}`);
}
...
export async function decideMeetingAgendaAction(formData: FormData) {
  const id = text(formData, "id");
  await decideMeetingAgenda({
    actorLabel: text(formData, "actorLabel"),
    agendaNo: text(formData, "agendaNo"),
    conditions: text(formData, "conditions"),
    documentId: id,
    meetingDate: text(formData, "meetingDate"),
    result: text(formData, "result") as "APPROVED" | "REJECTED" | "DEFERRED",
...
export async function createContractAction(formData: FormData) {
  const id = text(formData, "id");
  await createContractFromApproval(
    id,
    text(formData, "actorLabel"),
    text(formData, "paymentTerms"),
  );
  revalidatePath(`/approval/${id}`);
}
export async function createContractPaymentExpenseAction(formData: FormData) {
  const id = text(formData, "id");
  await createContractPaymentExpense(
    id,
    text(formData, "paymentId"),
    text(formData, "actorLabel"),
  );
  revalidatePath(`/approval/${id}`);
  revalidatePath("/finance/exp");
...
export async function updateApprovalAction(formData: FormData) {
  const id = text(formData, "id");
  await updateApprovalDocument(id, text(formData, "actorLabel"), {
...
export async function openApprovalAttachmentAction(formData: FormData) {
  redirect(
    await createApprovalAttachmentDownloadUrl(text(formData, "attachmentId")),
  );
}
