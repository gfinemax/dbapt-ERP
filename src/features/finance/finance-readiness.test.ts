...
function database() {
  return { schema: () => ({ rpc: vi.fn().mockImplementation((name: string) => Promise.resolve({ data: name === "collection_ledger_read" ? { assessments: [{ assessed_amount: 100, allocated_amount: 50 }, { assessed_amount: 100, allocated_amount: 100 }], refunds: [{ status: "DRAFT" }, { status: "PAID" }] } : [{ needs_review: true }, { needs_review: false }], count: null, error: null })), from: (table: string) => {
...
describe("finance operational readiness", () => {
  ...
  it("keeps configuration gaps separate from source-backed work queues", async () => {
    ...
    expect(result.queues).toEqual({ cardLinkPending: 5, evidencePending: 2, resolutionRequired: 1, personalPaymentPending: 3, advanceSettlementOpen: 2, operatingPeriodOpen: 1, routeUnclassified: 6, budgetReviewPending: 1, collectionPending: 1, refundPending: 1 });
  });
