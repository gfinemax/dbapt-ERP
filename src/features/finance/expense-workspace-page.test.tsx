...
vi.mock("@/app/finance/expenses/actions", () => ({
  attachQuickExpenseEvidenceAction: mocks.attach,
  connectExpenseOriginal: mocks.connect,
  reviewQuickExpenseEvidenceAction: mocks.review,
  updatePersonalReimbursementDetailsAction: mocks.personalUpdate,
  updateQuickExpenseDetailsAction: mocks.update,
}));
vi.mock("@/app/finance/expense-resolutions/actions", () => ({
...
function fixture(): ExpenseWorkspace {
  ...
  return {
    viewer: { staff: true, permissions: ["ADMIN"] },
    records: [
      base,
      {
        ...base,
        source_kind: "QUICK",
        source_id: "same-text-id",
        number: null,
        title: "개인 사용",
        approval_status: "CONVERTED",
        payment_status: null,
        transaction_id: "connected",
        can_connect: false,
      },
    ],
  };
}
const expenseDetails: OperatingExpenseDetail[] = [
  {
    id: "detail-supplies",
    code: "GENERAL-SUPPLIES",
    groupName: "일반운영비",
    name: "사무용품비",
    budgetItem: "일반운영비>사무용품비",
    status: "CONFIRMED",
    quickExpenseEligible: true,
  },
];
beforeEach(() => {
  vi.clearAllMocks();
  ...
  mocks.personalUpdate.mockResolvedValue({
    ok: true,
    message: "거래처와 사용내용을 수정했고 변경 이력을 남겼어.",
  });
  mocks.attach.mockResolvedValue({ id: "same-text-id" });
  mocks.review.mockResolvedValue({ id: "same-text-id" });
...
describe("common original expense workspace", () => {
  ...
  it("links an eligible approver directly to the personal reimbursement review", () => {
    const data = fixture();
    data.viewer.permissions = ["APPROVE"];
    data.records[0] = {
      ...data.records[0],
      source_kind: "PERSONAL",
      source_id: "personal-review",
      budget_month: "2026-03-01",
      approval_status: "SUBMITTED",
      payment_status: null,
      personal_is_applicant: false,
    };
    render(
      <ExpenseWorkspacePage
        workspace={data}
        initialSourceKind="PERSONAL"
        initialSourceId="personal-review"
      />,
    );
    expect(screen.getByText("승인 후 지급 연결")).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: "정산 승인 검토" }),
    ).toHaveAttribute(
      "href",
      "/finance/reimbursements?tab=requests&request=personal-review&month=2026-03&action=APPROVE#reimbursement-request-personal-review",
    );
  });
  it("does not offer an applicant approval of their own reimbursement", () => {
    ...
    data.records[0] = {
      ...data.records[0],
      source_kind: "PERSONAL",
      source_id: "own-request",
      budget_month: "2026-03-01",
      approval_status: "SUBMITTED",
      payment_status: null,
      personal_is_applicant: true,
    };
    render(
      <ExpenseWorkspacePage
        workspace={data}
        initialSourceKind="PERSONAL"
        initialSourceId="own-request"
      />,
    );
    expect(
      screen.queryByRole("link", { name: "정산 승인 검토" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: "개인 정산 신청 화면에서 확인" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText("승인 담당자의 처리를 기다리고 있습니다."),
    ).toBeInTheDocument();
  });
  it("links an approved reimbursement to payment for a payment operator", () => {
    const data = fixture();
    data.viewer.permissions = ["PAY"];
    data.records[0] = {
      ...data.records[0],
      source_kind: "PERSONAL",
      source_id: "personal-payment",
      budget_month: "2026-03-01",
    ...
    render(
      <ExpenseWorkspacePage
        workspace={data}
        initialSourceKind="PERSONAL"
        initialSourceId="personal-payment"
      />,
    );
    expect(screen.getByText("지급 거래 연결 대기")).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: "실제 출금 거래 연결" }),
    ).toHaveAttribute(
      "href",
      "/finance/reimbursements?tab=requests&request=personal-payment&month=2026-03&action=PAY#reimbursement-request-personal-payment",
    );
  });
  it("keeps the original reimbursement budget month when opening its existing page", () => {
    ...
    expect(
      screen.getByRole("link", { name: "기존 개인 대납 정산 화면에서 확인" }),
    ).toHaveAttribute("href", "/finance/reimbursements?month=2026-03");
  });
  it("filters original kind, connection and search with matching counts", () => {
    render(<ExpenseWorkspacePage workspace={fixture()} />);
    fireEvent.click(screen.getByRole("button", { name: "간편지출 1" }));
    expect(
      screen.getByText("전체 원본 2건 · 조회 결과 1건"),
    ).toBeInTheDocument();
    expect(
      within(screen.getByRole("table")).queryByText("사무용품"),
    ).not.toBeInTheDocument();
    fireEvent.change(screen.getByLabelText("업무흐름 연결"), {
      target: { value: "UNCONNECTED" },
    });
    expect(
      screen.getByText("전체 원본 2건 · 조회 결과 0건"),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "전체 2" }));
    fireEvent.change(screen.getByLabelText("지출 검색"), {
      target: { value: "지결-2026-1" },
    });
    expect(
      screen.getByText("전체 원본 2건 · 조회 결과 1건"),
    ).toBeInTheDocument();
  ...
  it("explains a pending card source without exposing the internal status code", () => {
    const data = fixture();
    data.records[1] = { ...data.records[1], approval_status: "SOURCE_PENDING" };
    render(
      <ExpenseWorkspacePage
        workspace={data}
    ...
    expect(
      table.queryByText("법인카드 승인내역이 들어오면 실제 거래와 연결해줘."),
    ).not.toBeInTheDocument();
    expect(
      screen.getAllByText(/법인카드 승인내역이 들어오면 실제 거래와 연결해줘/),
    ).toHaveLength(2);
    expect(detail.getAllByText("카드내역 연결대기").length).toBeGreaterThan(0);
    expect(
      detail.getByText("법인카드 승인내역이 들어오면 실제 거래와 연결해줘."),
    ).toBeInTheDocument();
    expect(screen.queryByText("SOURCE_PENDING")).not.toBeInTheDocument();
    expect(
      screen.getByRole("columnheader", { name: "업무흐름 연결" }),
    ).toBeInTheDocument();
  ...
  it("opens its own ID-specific detail and preserves legacy completion without inventing paid amounts", () => {
    ...
    expect(
      detail.getByRole("link", { name: "기존 지출결의 화면에서 확인" }),
    ).toHaveAttribute(
      "href",
      "/finance/expense-resolutions?resolutionId=text-id",
    );
  ...
  it("connects the existing source once and reads back the saved transaction without duplicate original rows", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "원본 연결" }),
    ).not.toBeInTheDocument();
    expect(screen.getByText(/누적 실제 지급 확인 필요/)).toBeInTheDocument();
    expect(
      screen.getByText("전체 원본 2건 · 조회 결과 2건"),
    ).toBeInTheDocument();
  });
  it("keeps selected original and retries the same key on failure", async () => {
    ...
    fireEvent.click(screen.getByRole("tab", { name: "연결 현황" }));
    fireEvent.click(screen.getByRole("button", { name: "원본 연결" }));
    await screen.findByText("일시적 연결 실패");
    await waitFor(() =>
      expect(
        screen.getByRole("button", { name: "원본 연결" }),
      ).not.toBeDisabled(),
    );
    fireEvent.click(screen.getByRole("button", { name: "원본 연결" }));
  ...
  it("prevents a double click while the enrollment is unresolved", async () => {
    ...
    fireEvent.click(screen.getByRole("tab", { name: "연결 현황" }));
    const button = screen.getByRole("button", { name: "원본 연결" });
  ...
  it("keeps document links on supported query parameters and separates original usage from payment", () => {
    ...
    workspace.records[0].trust_items = [
      {
        id: "item",
    ...
    fireEvent.click(screen.getByRole("tab", { name: "연결 현황" }));
    expect(screen.getByRole("link", { name: "회계-1" })).toHaveAttribute(
      "href",
      "/finance?voucherId=voucher-1",
    );
    expect(screen.getByRole("link", { name: "신탁-1" })).toHaveAttribute(
      "href",
      "/finance/trust?request=request-1",
    );
  ...
  it("starts from real existing forms and explicitly leaves unified advance settlement unfinished", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "이미 사용하거나 지급한 거래" }),
    );
    expect(
      screen.getByRole("link", { name: "결의 생략 근거가 있는 간편지출 등록" }),
    ).toHaveAttribute("href", "/finance/quick-expenses");
    fireEvent.click(
      screen.getByRole("button", { name: "먼저 지급한 돈의 정산" }),
    );
    expect(
      screen.getByText(
        /정산 확정·예산 반영·추가 지급 실행은 정책 확인 전까지 제한/,
      ),
    ).toBeInTheDocument();
    expect(
  ...
  it("does not offer staff drafting and accounting links to ordinary applicants", () => {
    const workspace = fixture();
    workspace.viewer = { staff: false, permissions: [] };
    workspace.records = [
      {
        ...workspace.records[0],
        source_kind: "PERSONAL",
    ...
    expect(
      screen.getByRole("link", { name: "개인이 먼저 쓴 경비 정산 신청" }),
    ).toHaveAttribute("href", "/finance/reimbursements");
    expect(
      screen.queryByRole("link", { name: /간편지출 등록/ }),
    ).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("tab", { name: "연결 현황" }));
    expect(
      screen.queryByRole("link", { name: "제목으로 지급 목록 확인" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "원본 연결" }),
    ).toBeInTheDocument();
  ...
  it("moves through the filtered list without closing the detail panel", () => {
    ...
    expect(screen.getByRole("button", { name: "이전 지출" })).toBeDisabled();
    fireEvent.click(screen.getByRole("button", { name: "다음 지출" }));
    expect(
    ...
    expect(
      screen.getByRole("heading", { name: "사무용품" }),
    ).toBeInTheDocument();
  });
  it("hides the detail area initially and reveals it only after selecting an expense", () => {
    ...
    expect(
      screen.queryByText(
        "목록에서 원본을 선택하면 상세와 연결 현황을 확인할 수 있어.",
      ),
    ).not.toBeInTheDocument();
  ...
  it("shows small expenses as a first-class source and opens their original detail", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "소액지출 1" }));
    expect(
      screen.getByText("전체 원본 3건 · 조회 결과 1건"),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "복사용지" }));
    expect(
      screen.getByRole("link", { name: "소액지출 상세 · 처리" }),
    ).toHaveAttribute(
      "href",
      "/finance/expenses/small?month=2026-09&id=small-1",
    );
  });
  ...
  it("reviews saved receipt OCR before updating quick-expense text and preserves the original amount", async () => {
      ...
      evidence_files: [
        {
          ...
          storage_path: "org/user/receipt.jpg",
          evidence_type: "영수증",
          status: "COMPLETED",
          stage: "COMPLETED",
          progress: 100,
          result_data: {
            issuer: "(주)아성다이소봉천본점",
            totalAmount: 15000,
            items: [{ itemName: "서류꽂이" }, { itemName: "건전지" }],
          },
          error_message: null,
    ...
    expect(
      screen.getByText(/원본 14,000원과 OCR 금액 15,000원이 달라/),
    ).toBeInTheDocument();
    fireEvent.click(
      screen.getByRole("button", { name: "OCR 결과를 내용 수정에 반영" }),
    );
    expect(screen.getByLabelText("사용내용")).toHaveValue("서류꽂이, 건전지");
    expect(screen.getByLabelText("거래처")).toHaveValue(
      "(주)아성다이소봉천본점",
    );
    ...
    await waitFor(() =>
      expect(mocks.update).toHaveBeenCalledWith({
        id: "same-text-id",
        usageDescription: "서류꽂이, 건전지",
        counterparty: "(주)아성다이소봉천본점",
        budgetItem: "일반운영비>사무용품비",
        expenseDetailId: "detail-supplies",
  ...
  it("records an explicit decision for alternative evidence", async () => {
    ...
    workspace.records[1] = {
      ...workspace.records[1],
      evidence_review_status: "REVIEW_REQUIRED",
      missing_evidence_reason: "영수증 분실",
      evidence_files: [
        {
          ocr_job_id: "job-1",
          file_name: "주문내역.jpg",
          content_type: "image/jpeg",
          storage_path: "org/user/order.jpg",
          evidence_type: "주문내역",
          status: "COMPLETED",
          stage: "COMPLETED",
          progress: 100,
    ...
    fireEvent.change(screen.getByLabelText("증빙 처리 사유"), {
      target: { value: "카드 승인내역과 주문내역 확인" },
    });
  ...
  it("edits an eligible personal source without changing protected amount and date fields", async () => {
    ...
    workspace.records[0] = {
      ...
      counterparty: "기존 거래처",
      approval_status: "SUBMITTED",
      personal_purpose: "기존 내용",
      personal_updated_at: "2026-09-18T00:00:00Z",
      personal_can_edit: true,
    };
    render(
      <ExpenseWorkspacePage
        workspace={workspace}
        initialSourceKind="PERSONAL"
        initialSourceId="personal"
      />,
    );
    fireEvent.click(
      screen.getByRole("button", { name: "거래처·사용내용 수정" }),
    );
    fireEvent.change(screen.getByLabelText("거래처"), {
      target: { value: "새 거래처" },
    });
    fireEvent.change(screen.getByLabelText("사용내용"), {
      target: { value: "새 사용내용" },
    });
    ...
    fireEvent.click(screen.getByRole("button", { name: "수정 저장" }));
    await waitFor(() =>
      expect(mocks.personalUpdate).toHaveBeenCalledWith({
        id: "personal",
        merchant: "새 거래처",
        purpose: "새 사용내용",
        reason: "상호 오기",
    ...
    expect(mocks.personalUpdate.mock.calls[0][0]).not.toHaveProperty("amount");
    expect(mocks.personalUpdate.mock.calls[0][0]).not.toHaveProperty("usedOn");
  });
  it("shows a safe personal edit error without refreshing stale data", async () => {
    mocks.personalUpdate.mockResolvedValue({
      ok: false,
      message: "변경된 거래처 또는 사용내용이 없습니다.",
    });
    const workspace = fixture();
    workspace.records[0] = {
      ...workspace.records[0],
    ...
    fireEvent.click(screen.getByRole("button", { name: "수정 저장" }));
    expect(
      await screen.findByText("변경된 거래처 또는 사용내용이 없습니다."),
    ).toBeInTheDocument();
