...
export function FinanceWorkspacePage({ workspace, initialTask = "ALL" }: { workspace: FinanceTaskWorkspace; initialTask?: string }) {
  ...
  const counts = useMemo(() => workspace.tasks.reduce<Partial<Record<FinanceTaskKind, number>>>((result, task) => {
    result[task.kind] = (result[task.kind] ?? 0) + 1;
    return result;
  }, {}), [workspace.tasks]);
  const rows = useMemo(() => {
    const normalizedSearch = search.trim().toLocaleLowerCase();
    return workspace.tasks.filter(row => (selected === "ALL" || row.kind === selected)
      && `${row.title} ${row.detail}`.toLocaleLowerCase().includes(normalizedSearch));
  }, [search, selected, workspace.tasks]);
