...
import { unconnectedPaymentRows } from "./payment-navigation";
...
export function FundPaymentPage({ workspace, trust, initialTab = "ALL", initialSearch = "", legacyRecords, legacyError }: { workspace: PaymentWorkspace; trust: TrustReadResult; initialTab?: string; initialSearch?: string; legacyRecords?: ExpenseWorkspaceRecord[]; legacyError?: string }) {
  ...
  const legacyRows = unconnectedPaymentRows(legacyRecords ?? [], tab, search);
  ...
  return <div className="space-y-5"><header className={card}><h1 className="text-3xl font-bold">지급관리</h1><p className="mt-2 text-slate-600">내부 승인·신탁 조건과 실제 지급을 구분해서 확인해. 날짜는 한국 시간 기준이야.</p><div className="mt-3 flex flex-wrap gap-3"><Link href="/finance/trust" className={secondary}>신탁 요청·집행 경로 확인</Link><Link href="/finance/reimbursements" className={secondary}>개인 대납 정산</Link></div></header>
    ...
    {(legacyRecords || legacyError) && <section className={card} aria-label="미연결 기존 지급 기록"><h2 className="text-lg font-bold">통합 연결 전 기존 기록</h2><p className="mt-2 text-sm">기존 지급 상태를 그대로 보존한 목록이야. 위 통합 지출 건수와 별도로 표시하고, 원본을 연결한 뒤 실제 지급 근거를 확인해.</p>{legacyError ? <p role="alert">{legacyError}</p> : <><p className="my-3">{paymentTabs[tab]} · 검색 결과 {legacyRows.length}건</p><ul className="divide-y">{legacyRows.map(row => <li className="py-3" key={`${row.source_kind}:${row.source_id}`}><Link className="font-semibold underline" href={`/finance/expenses?source_kind=${encodeURIComponent(row.source_kind)}&source_id=${encodeURIComponent(row.source_id)}`}>{row.number ? `${row.number} · ` : ""}{row.title}</Link><p className="mt-1 text-sm">{row.payment_status ?? row.approval_status} · 원본 금액 {money(row.amount)} · 실제 지급액은 원본 근거 확인</p></li>)}</ul></>}</section>}
