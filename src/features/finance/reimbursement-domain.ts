...
export type ReimbursementBudget = { id: string; budget_item: string; monthly_amount: number; approved_amount: number; annual_recorded_amount: number; quick_amount: number; personal_amount: number; unpaid_amount: number; reserved_amount: number };
export type ReimbursementReport = { month: string; revision: number; created_at: string; reason: string; snapshot: { budgets: ReimbursementBudget[]; requests: Pick<Reimbursement, "id" | "used_on" | "amount" | "budget_id" | "status">[] } };
...
export const reimbursementCommandLabels: Record<string,string> = { SUBMIT: "정산 신청", EXCEPTION: "지연 정산 승인", SENIOR: "장기 지연 승인", OVER_BUDGET: "예산 초과 승인", APPROVE: "예산 반영 승인", REJECT: "반려", CANCEL: "신청 취소", PAY: "지급 연결", REVERSE_PAYMENT: "지급 연결 취소", OPEN: "접수월 개설", SUPPLEMENT: "보완 접수", CLOSE: "월 마감", POLICY: "운영 기준 변경", MEMBER: "담당자 권한 변경" };
...
export function budgetUsed(b: ReimbursementBudget) { return Number(b.quick_amount) + Number(b.personal_amount); }
