...
function ExpenseStart({ staff }: { staff: boolean }) {
  ...
  return <section className={card}><h2 className="text-xl font-bold">지출 등록 시작</h2><p className="mt-2 text-sm text-slate-600">진행 상황을 선택한 다음 기존 작성 화면에서 등록해. 이미 등록한 건은 아래 원본 목록에서 연결해줘.</p>
    ...
    {scenario === "ADVANCE" && <div className="space-y-3"><p>원래 지급한 금액과 사용내역을 연결하고, 잔액 반납이나 추가 지급을 구분해야 해.</p><p className="rounded-lg bg-amber-50 p-3">선지급금의 원지급·사용액·잔액을 통합 정산하는 기능은 구현 중이야. 현재는 기존 지급·결의 자료를 확인할 수 있어.</p>{staff && <div className="flex flex-wrap gap-3"><Link className={secondary} href="/finance/payments">실제 지급 내역 확인</Link><Link className={secondary} href="/finance/expense-resolutions">기존 결의 자료 확인</Link></div>}</div>}
  </section>;
}
...
function ExpenseDetail({ record: r, staff }: { record: ExpenseWorkspaceRecord; staff: boolean }) {
  ...
  return <section aria-label="지출 상세" className={card}><h2 className="text-xl font-bold">{r.title}</h2><p className="mt-2 text-sm">{kinds[r.source_kind]}{r.number ? ` · ${r.number}` : ""}</p>
    ...
    <div className="mt-5 flex flex-wrap gap-3">{staff && <Link className={secondary} href={`/finance/payments?tab=ALL&q=${encodeURIComponent(r.title)}`}>제목으로 지급 목록 확인</Link>}{(staff || r.source_kind === "PERSONAL") && <Link className={secondary} href={r.source_kind === "RESOLUTION" ? "/finance/expense-resolutions" : r.source_kind === "QUICK" ? "/finance/quick-expenses" : `/finance/reimbursements${r.budget_month ? `?month=${r.budget_month}` : ""}`}>기존 {kinds[r.source_kind]} 화면에서 확인</Link>}</div><p className="mt-3 text-xs text-slate-600">기존 작성·승인·출력 기능은 각 원본 화면에 있어. 해당 화면의 목록에서 문서번호나 제목으로 찾아줘.</p>
  </section>;
}
