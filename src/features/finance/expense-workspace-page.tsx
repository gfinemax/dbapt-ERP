...
const kinds = { RESOLUTION: "지출결의", SMALL: "소액지출", QUICK: "간편지출", PERSONAL: "개인 대납 정산" };
const labels: Record<string, string> = { RECORDED: "간편처리 완료", SOURCE_PENDING: "카드내역 연결대기", EVIDENCE_PENDING: "증빙 확인대기", NEEDS_RESOLUTION: "결의 필요", CONVERTED: "결의·정산 연결", SUBMITTED: "승인대기", APPROVED: "승인완료", PAID: "지급완료", REJECTED: "반려", CANCELLED: "등록 취소", PENDING: "조합장 확인대기", RETURNED: "보완 필요", CONFIRMED: "확정", LEGACY_BATCH: "기존 일괄결의 연결", REVIEWING: "심사 중", PARTIAL: "일부 승인", SUPPLEMENT: "보완 요청", WITHDRAWAL_PENDING: "철회 확인 대기", WITHDRAWN: "철회" };
...
function QuickExpenseTools({ canApprove, expenseDetails, record: r }: { canApprove: boolean; expenseDetails: OperatingExpenseDetail[]; record: ExpenseWorkspaceRecord }) {
  ...
  return <section className="mt-5 border-t border-slate-200 pt-5" aria-label="간편지출 수정 및 영수증 OCR">
...
function displayPaymentStatus(r: ExpenseWorkspaceRecord) {
  ...
  if (r.source_kind === "SMALL") return r.approval_status === "CONFIRMED" ? "확정 후 지출원본 연결" : "확정 전";
...
function ExpenseStart({ staff }: { staff: boolean }) {
  ...
  return <details className={card}><summary className="cursor-pointer font-semibold">등록 방식이 궁금하다면</summary><h2 className="mt-4 text-xl font-bold">지출 등록 시작</h2><div className="mt-3 flex flex-wrap gap-3"><Link className={secondary} href="/finance/expenses/small">소액지출 등록·보완</Link><Link className={secondary} href="/approval/inbox?type=expense">지출 승인함</Link></div><p className="mt-2 text-sm text-slate-600">진행 상황을 선택한 다음 기존 작성 화면에서 등록해. 이미 등록한 건은 아래 원본 목록에서 연결해줘.</p>
    ...
    {scenario === "USED" && <div className="space-y-3"><p>개인 돈으로 먼저 지출한 경우와 조합 계좌·카드에서 이미 지급한 경우를 구분해. 예산 안이라는 사실만으로 결의 생략을 확정하지 않아.</p><div className="flex flex-wrap gap-3"><Link className={secondary} href="/finance/reimbursements">개인이 먼저 쓴 경비 정산 신청</Link>{staff && <><Link className={secondary} href="/finance/expenses/small">조합장 확인이 필요한 소액지출 등록</Link><Link className={secondary} href="/finance/quick-expenses">결의 생략 근거가 있는 간편지출 등록</Link><Link className={secondary} href={expenseResolutionHref({ start: "reimbursement" })}>사후 승인이 필요한 결의 작성</Link></>}</div></div>}
...
function ExpenseDetail({ canApprove, expenseDetails, onClose, permissions, record: r, staff }: { canApprove: boolean; expenseDetails: OperatingExpenseDetail[]; onClose: () => void; permissions: ExpenseWorkspace["viewer"]["permissions"]; record: ExpenseWorkspaceRecord; staff: boolean }) {
  ...
  return <section aria-label="지출 상세" className={`${card} @container/detail ${styles.detailCard}`}><div className={styles.detailHeader}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-sm font-semibold text-blue-700">{kinds[r.source_kind]}{r.number ? ` · ${r.number}` : ""}</p><h2 className="mt-1 break-words text-xl font-bold">{r.title}</h2><div className="mt-3 flex flex-wrap gap-2"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${r.approval_status === "SOURCE_PENDING" ? "bg-amber-100 text-amber-900" : "bg-blue-50 text-blue-700"}`}>{labels[r.approval_status] ?? r.approval_status}</span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">{r.transaction_id ? "업무흐름 연결됨" : "업무흐름 미연결"}</span>{r.payment_status && <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">{r.payment_status}</span>}</div>{r.approval_status === "SOURCE_PENDING" && <p className="mt-2 text-sm text-amber-800">{sourcePendingDescription}</p>}</div><button aria-label="지출 상세 닫기" className={`${secondary} shrink-0`} onClick={onClose}>목록으로</button></div>
    ...
    {tab === "DETAIL" && <div aria-labelledby="expense-detail-tab" id="expense-detail-panel" role="tabpanel"><dl className="my-5 grid gap-4 sm:grid-cols-2">{[["원본 승인·처리 상태", labels[r.approval_status] ?? r.approval_status], ["원본 지급 상태", displayPaymentStatus(r)], ["작성자", r.author_label ?? "미확인"], ["거래처", r.counterparty || "미확인"], ["작성일", day(r.created_at)], ["실제 사용일", day(r.used_at)], ["회계 귀속일", day(r.accounting_date)], ["개인 정산 예산월", r.budget_month?.slice(0, 7) ?? "해당 없음"]].map(([label, value]) => <div key={label}><dt className="text-sm text-slate-600">{label}</dt><dd className="mt-1 font-medium">{value}</dd></div>)}</dl>
      {staff && r.source_kind === "RESOLUTION" && <section className="mt-5 rounded-xl border border-blue-200 bg-blue-50/40 p-4"><h3 className="font-bold">결의서 관리</h3><p className="mt-1 text-sm text-slate-600">선택한 결의서의 내용·증빙·결재 이력을 확인하고, 진행상태와 권한에 따라 수정하거나 출력할 수 있어.</p><Link className={`${button} mt-3 inline-block`} href={expenseResolutionHref({ resolutionId: r.source_id })}>결의서 상세 · 수정 · 출력</Link></section>}
      {staff && r.source_kind === "SMALL" && <section className="mt-5 rounded-xl border border-blue-200 bg-blue-50/40 p-4"><h3 className="font-bold">소액지출 관리</h3><p className="mt-1 text-sm text-slate-600">등록 내용과 증빙을 확인하고, 권한과 처리상태에 따라 보완하거나 확정할 수 있어.</p><Link className={`${button} mt-3 inline-block`} href={`/finance/expenses/small?month=${r.used_at?.slice(0, 7) ?? ""}&id=${encodeURIComponent(r.source_id)}`}>소액지출 상세 · 처리</Link></section>}
    ...
    {tab === "CONNECTIONS" && <div aria-labelledby="expense-connections-tab" className="mt-5" id="expense-connections-panel" role="tabpanel">{r.transaction_id && r.amounts ? <div className="rounded-lg bg-slate-50 p-4"><h3 className="font-semibold">연결된 지급 현황</h3><p className="mt-2">누적 실제 지급 {money(r.amounts.paid)} · 총 미지급 {money(r.amounts.remaining)} · 승인 중 미지급 {money(r.amounts.approved_unpaid)}</p>{r.amounts.legacy_payment_complete && <p className="mt-2 text-sm">기존 지급완료 기록을 보존했어. 금액 근거가 없으면 확인 필요로 표시돼.</p>}</div> : <div className="rounded-lg bg-slate-50 p-4"><p>통합 업무에 아직 연결되지 않은 원본이야. 연결해도 원본을 복제하거나 새 지급·비용을 만들지 않아.</p>{r.can_connect && <button className={`${button} mt-3`} disabled={pending} onClick={connect}>원본 연결</button>}</div>}
      ...
      <div className="mt-5 flex flex-wrap gap-3">{staff && <Link className={secondary} href={`/finance/payments?tab=ALL&q=${encodeURIComponent(r.title)}`}>제목으로 지급 목록 확인</Link>}{(staff || r.source_kind === "PERSONAL") && <Link className={secondary} href={r.source_kind === "RESOLUTION" ? expenseResolutionHref({ resolutionId: r.source_id }) : r.source_kind === "SMALL" ? `/finance/expenses/small?month=${r.used_at?.slice(0, 7) ?? ""}&id=${encodeURIComponent(r.source_id)}` : r.source_kind === "QUICK" ? "/finance/quick-expenses" : `/finance/reimbursements${r.budget_month ? `?month=${r.budget_month.slice(0, 7)}` : ""}`}>기존 {kinds[r.source_kind]} 화면에서 확인</Link>}</div><p className="mt-3 text-xs text-slate-600">기존 작성·승인·출력 기능은 각 원본 화면에 있어. 선택한 유형의 원본 화면으로 이동해 후속 작업을 이어갈 수 있어.</p>
    </div>}
  </section>;
}

export function ExpenseWorkspacePage({ workspace, expenseDetails = [], initialKind = "ALL", initialConnection = "ALL", initialSearch = "", initialStatus = "", initialSourceKind, initialSourceId }: { workspace: ExpenseWorkspace; expenseDetails?: OperatingExpenseDetail[]; initialKind?: string; initialConnection?: string; initialSearch?: string; initialStatus?: string; initialSourceKind?: string; initialSourceId?: string }) {
  ...
  const rows = filterExpenseRecords(workspace.records, kind, connection, search).filter(r => kind !== "RESOLUTION" || !approvalStatus || r.approval_status === approvalStatus); const detail = workspace.records.find(r => keyOf(r) === selected);
  function navigate(next: { kind?: string; connection?: string; q?: string; status?: string; source_kind?: string; source_id?: string }) {
  ...
  useEffect(() => {
    ...
    if (panel.parentElement && getComputedStyle(panel.parentElement).gridTemplateColumns.split(" ").length === 1) {
  ...
  return <div className="space-y-5"><header className={card}><h1 className="text-3xl font-bold">지출관리</h1><p className="mt-2 text-slate-600">원본 지출과 승인·신탁·지급·회계 연결을 한곳에서 확인해.</p><nav aria-label="지출 업무" className="mt-5 flex flex-wrap items-center gap-3">
      {workspace.viewer.staff && <Link className={button} href={expenseResolutionHref({ start: "advance" })}>+ 지출결의 작성</Link>}
      {workspace.viewer.staff && <Link className={secondary} href="/finance/expenses/small">소액지출 등록</Link>}
      <Link className={secondary} href="/finance/reimbursements">개인 대납 정산</Link>
      {workspace.viewer.staff && <Link className="text-sm font-semibold text-blue-700 hover:underline" href="/approval/inbox?type=expense">통합 결재함 · 지출결의 →</Link>}
    </nav><p className="mt-3 text-sm text-slate-600">여기서 작성하고 진행상태를 확인해. 제출한 결의서는 통합 결재함에서 승인·반려해.</p>{!workspace.viewer.staff && <p className="mt-2">본인이 신청한 개인 대납 정산만 표시돼.</p>}</header><ExpenseStart staff={workspace.viewer.staff} />
    <div className={styles.workspace}><div className={styles.layout} data-selected={!!selected}><section className={`${card} ${styles.list}`}><h2 className="text-xl font-bold">지출 원본 목록</h2><p className="mt-2 text-sm text-slate-600">지출결의·소액지출·간편지출·개인 대납 정산을 함께 확인해. 전환된 원본은 중복 표시하지 않아.</p><div aria-label="지출 종류" className="mt-4 flex flex-wrap gap-2">
      {[["ALL", "전체"], ...Object.entries(kinds)].filter(([value]) => workspace.viewer.staff || value === "ALL" || value === "PERSONAL").map(([value, label]) => <button key={value} type="button" aria-pressed={kind === value} className={kind === value ? button : secondary} onClick={() => { setKind(value); setApprovalStatus(""); setSelected(""); navigate({ kind: value, status: "", source_kind: "", source_id: "" }); }}>{label} {workspace.records.filter(r => value === "ALL" || r.source_kind === value).length}</button>)}
    </div>
    {kind === "RESOLUTION" && <div className="mt-4 flex flex-wrap items-center gap-3"><label className="text-sm">결의 승인상태<select className={field} value={approvalStatus} onChange={e => { setApprovalStatus(e.target.value); navigate({ status: e.target.value }); }}><option value="">전체</option>{["작성중", "승인대기", "반려", "승인완료"].map(status => <option key={status}>{status}</option>)}</select></label><Link className="text-sm font-semibold text-blue-700 hover:underline" href="/finance/expense-resolutions">지출결의 전체 관리 · 출력 →</Link></div>}
    <div className="@container/filters"><div className="my-4 grid gap-3 @min-[400px]/filters:grid-cols-2 @min-[720px]/filters:grid-cols-3"><label>업무흐름 연결<select className={field} value={connection} onChange={e => { setConnection(e.target.value); navigate({ connection: e.target.value }); }}><option value="ALL">전체</option><option value="CONNECTED">연결됨</option><option value="UNCONNECTED">미연결</option></select></label><label className="@min-[400px]/filters:col-span-2 @min-[720px]/filters:col-span-1">지출 검색<input className={field} value={search} placeholder="제목·문서번호·거래처" onChange={e => { setSearch(e.target.value); navigate({ q: e.target.value }); }} /></label></div></div>
      <p className="mb-3">전체 원본 {workspace.records.length}건 · 조회 결과 {rows.length}건</p>{rows.some(r => r.approval_status === "SOURCE_PENDING") && <p className="mb-3 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800"><span className="font-semibold">카드내역 연결대기</span> · {sourcePendingDescription}</p>}<div className="overflow-x-auto"><table role="table" className={styles.table}><colgroup><col /><col className="w-28" /><col className="w-48" /><col className="w-36" /></colgroup><thead role="rowgroup"><tr role="row"><th role="columnheader" className="p-2">원본</th><th role="columnheader" className="px-2 text-right">금액</th><th role="columnheader" className="px-2">원본 상태</th><th role="columnheader" className="px-2">업무흐름 연결</th></tr></thead><tbody role="rowgroup">{rows.map(r => { const active = selected === keyOf(r); return <tr role="row" aria-selected={active} key={keyOf(r)} className={`cursor-pointer border-t align-top transition-colors hover:bg-blue-50/60 ${active ? "bg-blue-50 shadow-[inset_4px_0_0_#2563eb]" : ""}`} onClick={() => selectDetail(r)}><td role="cell" className="p-2"><button id={`expense-source-${r.source_kind}-${r.source_id}`} className="line-clamp-2 w-full text-left font-semibold underline decoration-slate-400 underline-offset-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600" aria-pressed={active} onClick={event => { event.stopPropagation(); selectDetail(r); }}>{r.title}</button><p className="mt-1 truncate text-xs">{kinds[r.source_kind]}{r.number ? ` · ${r.number}` : ""}</p></td><td role="cell" className="px-2 py-3 text-right font-medium tabular-nums whitespace-nowrap">{money(r.amount)}</td><td role="cell" className="px-2 py-2">{r.approval_status === "SOURCE_PENDING" ? <div><span className="inline-flex rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-900" title={sourcePendingDescription}>{labels[r.approval_status]}</span>{r.payment_status ? <p className="mt-1 text-xs">{r.payment_status}</p> : null}</div> : <>{labels[r.approval_status] ?? r.approval_status}{r.payment_status ? ` · ${r.payment_status}` : ""}</>}</td><td role="cell" className="px-2 py-2"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${r.transaction_id ? "bg-blue-50 text-blue-700" : "bg-slate-100 text-slate-700"}`}>{r.transaction_id ? "업무흐름 연결됨" : "업무흐름 미연결"}</span></td></tr>; })}</tbody></table></div>{!rows.length && <p className="py-6 text-center">조건에 맞는 지출 원본이 없어.</p>}</section>
      {selected && <aside ref={panelRef} tabIndex={-1} aria-label="선택한 지출 원본 상세 패널" className={styles.panel}>
        {detail ? <ExpenseDetail canApprove={workspace.viewer.permissions.some(p => ["ADMIN","APPROVE"].includes(p))} expenseDetails={expenseDetails} key={keyOf(detail)} onClose={closeDetail} permissions={workspace.viewer.permissions} record={detail} staff={workspace.viewer.staff} /> : <section className={card}><button className={secondary} onClick={closeDetail}>목록으로</button><p className="mt-4" role="alert">조회 권한이 있는 원본 목록에서 해당 지출을 찾을 수 없어.</p></section>}
      </aside>}
    </div></div>
  </div>;
}
