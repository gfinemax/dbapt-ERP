...
export function ExpenseWorkspacePage({ workspace, expenseDetails = [], initialKind = "ALL", initialConnection = "ALL", initialSearch = "", initialSourceKind, initialSourceId }: { workspace: ExpenseWorkspace; expenseDetails?: OperatingExpenseDetail[]; initialKind?: string; initialConnection?: string; initialSearch?: string; initialSourceKind?: string; initialSourceId?: string }) {
  ...
  return <div className="space-y-5"><header className={card}><h1 className="text-3xl font-bold">지출관리</h1><p className="mt-2 text-slate-600">원본 지출과 승인·신탁·지급·회계 연결을 한곳에서 확인해.</p>{!workspace.viewer.staff && <p className="mt-2">본인이 신청한 개인 대납 정산만 표시돼.</p>}</header><ExpenseStart staff={workspace.viewer.staff} />
    <div className="2xl:grid 2xl:grid-cols-[minmax(650px,1fr)_minmax(420px,480px)] 2xl:items-start 2xl:gap-5"><section className={card}><h2 className="text-xl font-bold">지출 원본 목록</h2><p className="mt-2 text-sm text-slate-600">같은 사용을 결의·정산으로 전환한 기록도 원본별로 보존해. 이 목록의 금액을 합산하면 중복될 수 있어.</p><div className="my-4 grid gap-3 sm:grid-cols-3"><label>원본 종류<select className={field} value={kind} onChange={e => { setKind(e.target.value); navigate({ kind: e.target.value }); }}><option value="ALL">전체</option>{Object.entries(kinds).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>통합 연결<select className={field} value={connection} onChange={e => { setConnection(e.target.value); navigate({ connection: e.target.value }); }}><option value="ALL">전체</option><option value="CONNECTED">연결됨</option><option value="UNCONNECTED">미연결</option></select></label><label>지출 검색<input className={field} value={search} placeholder="제목·문서번호·거래처" onChange={e => { setSearch(e.target.value); navigate({ q: e.target.value }); }} /></label></div>
      ...
      {detail && <aside aria-label="선택한 지출 원본 상세 패널" className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-slate-50 p-3 shadow-2xl sm:max-w-[520px] 2xl:sticky 2xl:top-4 2xl:z-auto 2xl:col-start-2 2xl:row-start-1 2xl:max-h-[calc(100vh-2rem)] 2xl:max-w-none 2xl:bg-transparent 2xl:p-0 2xl:shadow-none"><ExpenseDetail canApprove={workspace.viewer.permissions.some(p => ["ADMIN","APPROVE"].includes(p))} expenseDetails={expenseDetails} key={keyOf(detail)} onClose={closeDetail} record={detail} staff={workspace.viewer.staff} /></aside>}
      {!detail && selected && <aside className={`${card} fixed inset-x-3 top-24 z-50 sm:left-auto sm:w-[520px] 2xl:static 2xl:col-start-2 2xl:row-start-1 2xl:w-auto`}><button className={`${secondary} float-right`} onClick={closeDetail}>닫기</button><p className="pr-20" role="alert">조회 권한이 있는 원본 목록에서 해당 지출을 찾을 수 없어.</p></aside>}
    </div>
  </div>;
}
