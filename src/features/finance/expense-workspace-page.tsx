...
export function ExpenseWorkspacePage({ workspace, expenseDetails = [], initialKind = "ALL", initialConnection = "ALL", initialSearch = "", initialSourceKind, initialSourceId }: { workspace: ExpenseWorkspace; expenseDetails?: OperatingExpenseDetail[]; initialKind?: string; initialConnection?: string; initialSearch?: string; initialSourceKind?: string; initialSourceId?: string }) {
  ...
  return <div className="space-y-5"><header className={card}><h1 className="text-3xl font-bold">지출관리</h1><p className="mt-2 text-slate-600">원본 지출과 승인·신탁·지급·회계 연결을 한곳에서 확인해.</p>{!workspace.viewer.staff && <p className="mt-2">본인이 신청한 개인 대납 정산만 표시돼.</p>}</header><ExpenseStart staff={workspace.viewer.staff} />
    <div className="2xl:grid 2xl:grid-cols-[minmax(650px,1fr)_minmax(420px,480px)] 2xl:items-start 2xl:gap-5"><section className={card}><h2 className="text-xl font-bold">지출 원본 목록</h2><p className="mt-2 text-sm text-slate-600">같은 사용을 결의·정산으로 전환한 기록도 원본별로 보존해. 이 목록의 금액을 합산하면 중복될 수 있어.</p><div className="my-4 grid gap-3 sm:grid-cols-3"><label>원본 종류<select className={field} value={kind} onChange={e => { setKind(e.target.value); navigate({ kind: e.target.value }); }}><option value="ALL">전체</option>{Object.entries(kinds).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>업무흐름 연결<select className={field} value={connection} onChange={e => { setConnection(e.target.value); navigate({ connection: e.target.value }); }}><option value="ALL">전체</option><option value="CONNECTED">연결됨</option><option value="UNCONNECTED">미연결</option></select></label><label>지출 검색<input className={field} value={search} placeholder="제목·문서번호·거래처" onChange={e => { setSearch(e.target.value); navigate({ q: e.target.value }); }} /></label></div>
      <p className="mb-3">전체 원본 {workspace.records.length}건 · 조회 결과 {rows.length}건</p><div className="overflow-x-auto"><table className="w-full min-w-[650px] text-left text-sm"><thead><tr><th className="p-2">원본</th><th>금액</th><th>원본 상태</th><th>업무흐름 연결</th></tr></thead><tbody>{rows.map(r => { const active = selected === keyOf(r); return <tr aria-selected={active} key={keyOf(r)} className={`cursor-pointer border-t transition-colors hover:bg-blue-50/60 ${active ? "bg-blue-50 shadow-[inset_4px_0_0_#2563eb]" : ""}`} onClick={() => selectDetail(r)}><td className="p-2"><button id={`expense-source-${r.source_kind}-${r.source_id}`} className="w-full text-left font-semibold underline decoration-slate-400 underline-offset-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600" aria-pressed={active} onClick={event => { event.stopPropagation(); selectDetail(r); }}>{r.title}</button><p className="mt-1 text-xs">{kinds[r.source_kind]}{r.number ? ` · ${r.number}` : ""}</p></td><td>{money(r.amount)}</td><td>{r.approval_status === "SOURCE_PENDING" ? <div><span className="inline-flex rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-900">{labels[r.approval_status]}</span><p className="mt-1 max-w-56 text-xs text-amber-800">{sourcePendingDescription}</p>{r.payment_status ? <p className="mt-1 text-xs">{r.payment_status}</p> : null}</div> : <>{labels[r.approval_status] ?? r.approval_status}{r.payment_status ? ` · ${r.payment_status}` : ""}</>}</td><td>{r.transaction_id ? "업무흐름 연결됨" : "업무흐름 미연결"}</td></tr>; })}</tbody></table></div>{!rows.length && <p className="py-6 text-center">조건에 맞는 지출 원본이 없어.</p>}</section>
