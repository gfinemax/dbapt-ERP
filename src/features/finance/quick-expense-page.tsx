...
export function QuickExpensePage({ attachEvidence, discardEvidence, embedded = false, getEvidenceOcrJob, getPrintEvidence, retryEvidenceOcrJob, importCardTransactions, linkCardTransaction, initialBankTransactions, initialBudgetItems = [], initialExpenseDetails = [], initialCardTransactions, initialRecords, initialPaymentMethod = "BANK_TRANSFER", initialSourceId = "", persistRecord, uploadEvidence = uploadQuickExpenseEvidence }: {
  ...
  getPrintEvidence?: (recordId: string) => Promise<QuickExpensePrintEvidence[]>;
  ...
  const formattedManualAmount = manualAmount.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  ...
  const recordMonths = Array.from(new Set(records.map((record) => record.occurredAt.slice(0, 7)))).sort().reverse();
  const monthlyRecords = records.filter((record) => record.occurredAt.startsWith(printMonth));
  ...
  const content = <div className="mx-auto grid max-w-[1180px] gap-6">
    {!embedded ? <section className="rounded-2xl border border-[var(--color-soft-border)] bg-white p-6"><p className="text-xs font-bold text-[var(--color-deep-cobalt)]">회계/자금 &gt; 전표·증빙관리</p><h1 className="mt-2 text-3xl font-bold">예산 내 간편지출</h1><p className="mt-2 text-sm text-[var(--color-stone)]">정식 지출결의서를 만들지 않고 실제 거래에 사용내용과 예산항목을 기록합니다. 한도초과·계약·예산 외 거래는 자동으로 정식 결의 대상으로 분류합니다.</p></section> : null}
    <section className="grid gap-5 rounded-2xl border border-[var(--color-soft-border)] bg-white p-6">
      {!embedded ? <fieldset><legend className="text-sm font-bold">결제수단</legend><div className="mt-3 flex flex-wrap gap-2">{paymentMethodOrder.map((method) => <button aria-pressed={paymentMethod === method} className={`rounded-full border px-4 py-2 text-sm font-bold ${paymentMethod === method ? "border-[var(--color-deep-cobalt)] bg-[var(--color-morning-tint)]" : "border-[var(--color-soft-border)]"}`} key={method} onClick={() => { setPaymentMethod(method); setSourceId(""); setManualCardEntry(false); setExpenseDetailSelection("AUTO"); setExpenseDetailId(""); setBudgetItem(""); setMessage(""); }} type="button">{paymentLabels[method]}</button>)}</div></fieldset> : null}
    ...
    {!embedded ? <Link href="/finance/reimbursements" className="rounded-xl border bg-white p-4 text-sm font-semibold text-blue-700">개인 선지출은 개인 지출 정산에서 신청·지급 상태를 관리해. 정산 화면으로 이동 →</Link> : null}
  ...
  return <ErpShell activeDetailLabel="예산 내 간편지출" activeLabel="회계/자금" activeWorkspaceLabel="전표·증빙관리">{content}</ErpShell>;
}
