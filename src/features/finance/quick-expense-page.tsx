...
export function QuickExpensePage({ initialBankTransactions, initialCardTransactions, initialRecords, persistRecord }: {
  ...
  return <ErpShell activeDetailLabel="예산 내 간편지출" activeLabel="회계/자금" activeWorkspaceLabel="전표·증빙관리"><div className="mx-auto grid max-w-[1180px] gap-6">
    ...
    <section className="rounded-2xl border border-[var(--color-soft-border)] bg-white p-6"><h2 className="text-lg font-bold">최근 간편지출 기록</h2><div className="mt-4 grid gap-3">{records.length ? records.map((record) => <article className="rounded-xl border p-4" key={record.id}><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-bold">{record.usageDescription}</p><span className={`rounded-full px-3 py-1 text-xs font-bold ${record.recordStatus === "SOURCE_PENDING" ? "bg-amber-100 text-amber-900" : record.recordStatus === "RECORDED" ? "bg-[var(--color-sprout)] text-[var(--color-green-ink)]" : "bg-[var(--color-sunset-soft)] text-[var(--color-tangerine)]"}`}>{record.recordStatus === "RECORDED" ? "간편처리 완료" : record.recordStatus === "SOURCE_PENDING" ? "카드내역 연결대기" : "정식결의 필요"}</span></div><p className="mt-2 text-sm text-[var(--color-stone)]">{record.counterparty} · {record.amount.toLocaleString("ko-KR")}원 · {record.budgetItem}</p></article>) : <p className="text-sm text-[var(--color-stone)]">등록된 간편지출이 없습니다.</p>}</div></section>
  </div></ErpShell>;
}
