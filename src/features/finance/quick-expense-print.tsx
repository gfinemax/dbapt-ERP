...
type EvidencePage = QuickExpensePrintEvidence & { compact: boolean; page: number; pageCount: number; recordId: string; src: string };
...
function MonthlyPrintPages({ evidencePages, month, records }: { evidencePages: EvidencePage[]; month: string; records: QuickExpenseRecord[] }) {
  ...
  const evidenceSheets = groupMonthlyEvidencePages(evidencePages);
  const totalPages = summaryPageCount + evidenceSheets.length;
  ...
  return <>{(pages.length ? pages : [[]]).map((pageRecords, pageIndex) => <article className="erp-print-page expense-resolution-print-page mx-auto rounded-sm bg-white shadow-sm" key={pageIndex}>
    <header className="flex items-end justify-between gap-6 border-b-2 border-[var(--color-midnight-ink)] pb-5"><div><p className="mb-2 inline-flex rounded-full bg-[var(--color-cloud-veil)] px-2.5 py-1 text-[9px] font-bold">전자시스템 원본 기준 · 월별 검토용 출력본</p><h3 className="text-[29px] font-black tracking-[0.08em]">간편지출 월별 총괄표</h3><p className="mt-1.5 text-[15px] font-semibold text-[var(--color-stone)]">대방동 지역주택조합 · {month}</p></div><div className="text-right"><p className="text-[11px] font-bold">조회 {records.length}건 · 증빙 {new Set(evidencePages.map((page) => page.id)).size}건</p><p className="mt-1 text-[22px] font-black">{formatWon(total)}</p><p className="text-[9px] text-[var(--color-stone)]">정식결의 전환 {converted.length}건 제외</p></div></header>
    ...
    <section className="mt-5"><div className="mb-2 flex items-end justify-between"><h4 className="text-[15px] font-bold">간편지출 기록</h4><p className="text-[10px] text-[var(--color-stone)]">전환 건은 합계에서 제외 · 개별 기록서는 필요할 때만 건별 출력</p></div><table className="w-full table-fixed border-collapse text-[8px]"><colgroup><col className="w-[5%]"/><col className="w-[9%]"/><col className="w-[17%]"/><col className="w-[9%]"/><col className="w-[14%]"/><col className="w-[19%]"/><col className="w-[16%]"/><col className="w-[11%]"/></colgroup><thead><tr className="bg-[var(--color-cloud-veil)]">{["순번","사용일","관리번호","결제","거래처","사용내용·예산","거래·증빙","금액"].map((label)=><th className="border px-1 py-2" key={label}>{label}</th>)}</tr></thead><tbody>{pageRecords.map((record,index)=>{const hasEvidence=evidencePages.some((page)=>page.recordId===record.id);return <tr className={record.recordStatus === "CONVERTED" ? "text-[var(--color-stone)]" : ""} key={record.id}><td className="border p-1 text-center">{pageIndex*12+index+1}</td><td className="border p-1 text-center">{dateOnly(record.occurredAt).slice(5)}</td><td className="break-all border p-1 text-center">{getQuickExpensePrintNumber(record)}</td><td className="border p-1 text-center">{paymentLabels[record.paymentMethod]}</td><td className="border p-1">{record.counterparty}</td><td className="border p-1"><p className="font-semibold">{record.usageDescription}</p><p className="mt-0.5 text-[7px] text-[var(--color-stone)]">{record.budgetItem}</p></td><td className="border p-1 text-center"><p>{statusLabels[record.recordStatus]}</p><p className="mt-0.5 text-[7px] text-[var(--color-stone)]">{hasEvidence?"증빙 첨부":record.evidenceReviewStatus?reviewLabels[record.evidenceReviewStatus]:"증빙 미첨부"}</p></td><td className="border p-1 text-right font-bold">{formatWon(record.amount)}</td></tr>})}</tbody></table></section>
    ...
    <PrintFooter note="전자시스템 원본 기준 · 월별 검토용 출력본" number={`간편지출-${month}`} page={pageIndex + 1} total={totalPages} />
  </article>)}
  {evidenceSheets.map((sheet, sheetIndex) => <article className="erp-print-page expense-resolution-print-page mx-auto rounded-sm bg-white shadow-sm" key={`evidence-sheet-${sheetIndex}`}>
    <header className="flex items-end justify-between gap-6 border-b-2 border-[var(--color-midnight-ink)] pb-4"><div><h3 className="text-[22px] font-black tracking-[0.08em]">간편지출 영수증 첨부지</h3><p className="mt-1.5 text-[10px] font-semibold text-[var(--color-stone)]">{month} 월별 총괄표 순번·관리번호 기준</p></div><p className="text-[10px] font-bold text-[var(--color-stone)]">첨부 {sheetIndex+1} / {evidenceSheets.length}</p></header>
    <div className={`mt-5 grid flex-1 gap-4 ${sheet.length > 1 ? "grid-rows-2" : "grid-rows-1"}`}>{sheet.map((evidence) => {const record=records.find((item)=>item.id===evidence.recordId);const sequence=records.findIndex((item)=>item.id===evidence.recordId)+1;return <section className="flex min-h-0 flex-col overflow-hidden border border-[var(--color-soft-border)] p-3" key={`${evidence.id}-${evidence.page}`}><div className="flex items-start justify-between gap-4 border-b pb-2 text-[9px]"><div><p className="font-bold">순번 {sequence} · {record?getQuickExpensePrintNumber(record):evidence.recordId}</p><p className="mt-1 text-[var(--color-stone)]">{record?.counterparty??"-"} · {record?formatWon(record.amount):"-"} · {evidence.fileName}</p></div><p className="shrink-0 font-bold">{evidence.evidenceType} · {evidence.page}/{evidence.pageCount}</p></div><figure className="mt-3 flex min-h-0 flex-1 items-center justify-center overflow-hidden bg-white p-2">
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img alt={`${evidence.fileName} 월별 증빙 ${evidence.page}페이지`} className="max-h-full max-w-full object-contain" src={evidence.src}/>
    </figure></section>;})}</div>
    <PrintFooter note="전자시스템 원본 기준 · 월별 검토용 출력본" number={`간편지출-${month}`} page={summaryPageCount+sheetIndex+1} total={totalPages}/>
  </article>)}</>;
}
...
function PrintFooter({ note = "건별 기록서와 증빙을 함께 편철하여 보관합니다.", number, page, total }: { note?: string; number: string; page: number; total: number }) { return <footer className="mt-auto flex items-end justify-between gap-6 border-t pt-4 text-[9px] text-[var(--color-stone)]"><div><p className="text-[12px] font-bold text-[var(--color-midnight-ink)]">대방동 지역주택조합</p><p className="mt-1">{note}</p></div><p>{number} · 출력일 {new Date().toLocaleDateString("sv-SE", { timeZone: "Asia/Seoul" })} · {page} / {total}</p></footer>; }
...
async function prepareEvidencePages(evidence: QuickExpensePrintEvidence[], recordId: string) {
  ...
  for (const item of evidence) {
    ...
    if (contentType === "application/pdf" || item.fileName.toLowerCase().endsWith(".pdf")) {
      ...
      images.forEach((src, index) => pages.push({ ...item, compact: false, page: index + 1, pageCount: images.length, recordId, src }));
    } else if (contentType.startsWith("image/") || item.contentType.startsWith("image/")) { const blob=await response.blob(); pages.push({ ...item, compact: blob.size<=2_000_000, page: 1, pageCount: 1, recordId, src: await blobToDataUrl(blob) }); }
  }
  return pages;
}
...
function groupMonthlyEvidencePages(pages: EvidencePage[]) { const sheets: EvidencePage[][]=[]; let compact:EvidencePage[]=[]; const flush=()=>{if(compact.length){sheets.push(compact);compact=[];}}; pages.forEach((page)=>{if(!page.compact){flush();sheets.push([page]);return;} compact.push(page);if(compact.length===2)flush();});flush();return sheets; }
