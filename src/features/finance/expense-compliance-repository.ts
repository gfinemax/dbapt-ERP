...
export async function getExpenseComplianceSettings(organizationId: string): Promise<ExpenseComplianceSettings | null> {
  const supabase = requireSupabase();
  const { data, error } = await supabase.schema("finance").from("expense_compliance_settings").select("petty_cash_limit,monthly_person_warning_limit,petty_cash_allowed_accounts,petty_cash_excluded_keywords,allow_no_evidence_approval,no_evidence_approver_role,allow_personal_reimbursement,post_approval_max_days,fact_confirmer_roles,approval_line,allow_direct_expense,direct_expense_limit,direct_expense_required_keywords,direct_expense_recommended_keywords,allow_other_approval_skip_reason,quick_expense_allowed_budget_items").eq("organization_id", organizationId).maybeSingle();
  if (error) throw new Error(`지출 관리설정 조회 실패: ${error.message}`);
  return data ? { allowDirectExpense: data.allow_direct_expense ?? true, directExpenseLimit: Number(data.direct_expense_limit ?? 5_000_000), directExpenseRequiredKeywords: data.direct_expense_required_keywords ?? [], directExpenseRecommendedKeywords: data.direct_expense_recommended_keywords ?? [], quickExpenseAllowedBudgetItems: data.quick_expense_allowed_budget_items ?? [], allowOtherApprovalSkipReason: data.allow_other_approval_skip_reason ?? true, allowNoEvidenceApproval: data.allow_no_evidence_approval, allowPersonalReimbursement: data.allow_personal_reimbursement, approvalLine: data.approval_line, factConfirmerRoles: data.fact_confirmer_roles, monthlyPersonWarningLimit: Number(data.monthly_person_warning_limit), noEvidenceApproverRole: data.no_evidence_approver_role ?? undefined, pettyCashAllowedAccounts: data.petty_cash_allowed_accounts, pettyCashExcludedKeywords: data.petty_cash_excluded_keywords, pettyCashLimit: Number(data.petty_cash_limit), postApprovalMaxDays: data.post_approval_max_days ?? undefined } : null;
}
