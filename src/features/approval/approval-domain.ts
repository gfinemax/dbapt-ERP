...
export type ApprovalDocument = {
  ...
  paymentSchedule?: Array<{ amount: number; dueDate: string; memo: string }>;
  recommendedMeetingBody?: string;
...
export type ApprovalDraftInput = Pick<
  ApprovalDocument,
  | "amount"
  | "body"
  | "budgetItem"
...
> & {
  ...
  approvalLineRuleId?: string;
  contractRelated?: boolean;
  desiredExecutionDate?: string;
  evidenceKind?: string;
  expectedEffect?: string;
  finalMeetingBody?: string;
  contractStartDate?: string;
  contractEndDate?: string;
  contractPaymentTerms?: string;
  paymentSchedule?: Array<{ amount: number; dueDate: string; memo: string }>;
  ...
  lines?: ApprovalDraftLine[];
  meetingConfirmed?: boolean;
  memberBurden?: boolean;
  outOfBudget?: boolean;
  paymentDueDate?: string;
...
export function meetingStatusForAmount(
  amount: number,
...
export function canExecute(
  document: Pick<ApprovalDocument, "approvalStatus" | "meetingStatus">,
) {
  return (
...
export function calculateBudgetAvailability(
  ...
  return {
    ...
    available,
    executed,
    expectedBalance: available - request,
    request,
    reserved,
  };
}
...
export function reconcileReservation(
  ...
  return {
    released: approvedAmount - paidAmount,
    requiresReapproval: false,
    status: "CONSUMED" as const,
  };
}

export function isMaterialApprovalChange(
  before: {
    amount: number;
    budgetItem?: string;
    counterpartyName?: string;
  ...
  return (
    ...
    before.budgetItem !== after.budgetItem ||
    before.counterpartyName !== after.counterpartyName ||
    before.meetingStatus !== after.meetingStatus ||
    before.projectName !== after.projectName
  );
}

export function validateSmallExpensePolicy(input: {
  amount: number;
  description: string;
  limit: number;
  memo?: string;
  ...
  if (
    /계약|조합원\s*환불|차입|상환|소송|예산\s*외|추가\s*부담/.test(
      `${input.description} ${input.memo ?? ""}`,
    )
  )
    throw new Error("소액결의 제외 업무야.");
}
