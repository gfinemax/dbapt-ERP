...
export function SmallExpensePage({ workspace: w }: { workspace: SmallExpenseWorkspace }) {
  const router = useRouter(); const [pending, start] = useTransition(); const [message, setMessage] = useState("");
  const [selected, setSelected] = useState<string[]>([]); const [verified, setVerified] = useState(false); const [reason, setReason] = useState("");
  const [editing, setEditing] = useState<SmallExpense>();
  const summary = summarizeSmallExpenses(w.rows); const director = w.roles?.director_id === w.member.user_id;
  const chair = w.roles?.chair_id === w.member.user_id; const admin = w.member.permissions.includes("ADMIN");
  ...
  const canSelect = (r: SmallExpense) => chair ? canConfirmSmallExpense(r, w.member.user_id, w.roles) : director && ["PENDING", "RETURNED"].includes(r.reviewStatus) && !r.batchResolutionId;
  const eligible = w.rows.filter(canSelect);
  return <>
    <header className={box}><div className="flex flex-wrap justify-between gap-4"><div><h1 className="text-3xl font-bold">소액지출 확인·확정</h1><p className="mt-2 text-sm text-slate-600">건당 {money(w.limit)} 이하 · 사무국장 등록 → 조합장 확인·확정 → 월별 자동 집계</p><p className="mt-2 text-sm">{w.member.display_name} · {director ? "사무국장" : chair ? "조합장" : "관리자"}</p></div>
    <form className="flex items-center gap-2"><label>사용월 <input aria-label="사용월" className={input} type="month" name="month" defaultValue={w.month} required /></label><button className={button}>조회</button></form></div></header>
    ...
    <section className={box}><h2 className="text-lg font-bold">사용내역과 증빙</h2>
      ...
      <div className="mt-4 space-y-3">{w.rows.length ? w.rows.map(row => <article className="rounded-xl border border-slate-200 p-4" key={`${row.id}:${row.revision}`}>
        ...
        {director && canSelect(row) ? <button type="button" className="mt-3 text-sm underline" onClick={() => { setEditing(row); window.scrollTo({ top: 0, behavior: "smooth" }); }}>내역 보완</button> : null}
        </div></div></article>) : <p className="py-8 text-center text-slate-600">이 달에 등록된 소액지출이 없습니다.</p>}</div>
      {chair || director ? <div className="mt-5 space-y-3 border-t pt-4"><button className="text-sm underline" disabled={pending || !eligible.length} onClick={() => { setSelected(selected.length ? [] : eligible.map(r => r.id)); setVerified(false); }}>{selected.length ? "선택 해제" : "처리 가능한 내역 전체 선택"}</button><p className="font-bold">선택 {selectedRows.length}건 · {money(selectedRows.reduce((sum, r) => sum + r.amount, 0))}</p>
