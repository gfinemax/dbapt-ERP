...
2026-09-19 후속 구현에서 고유 외부 조합원 ID 기반 부과 원본, 실제 입금의 부분 배분·취소 이력, 원수납 기반 환급의 분리 승인·실제 출금 연결, 간편지출 증빙 조회, 통합 월 마감 준비 점검을 추가했다. 이어 UTF-8 CSV 부과자료를 행별로 신규·수정·변경 없음·오류로 미리보기한 뒤 원자적으로 적용하는 기능을 추가했다. 파일명과 SHA-256, 정규화한 행, 적용자를 보존하며 이름 자동 매칭을 하지 않는다. 파일 내부 중복, 취소 원본 충돌, 수납액 미만 수정, 미리보기 후 버전 변경은 적용 전에 차단한다. 외부 원장 자동 동기화, 전자세금계산서 발행·세무 원장, 회계기간 잠금·재개방은 여전히 별도 범위다. 회계정책 확인 전 마감 확정·재개방 버튼은 제공하지 않는다.
