...
> **현재 상태:** 2026-09-08 16:08 KST에 운영 DB 호환 변경과 workflow migration 12개를 하나의 transaction으로 적용했다. 16:11 KST에 rollback 전용 SQL 회귀검사 10개와 원본 수치 재대조를 통과했다. 16:23 KST에 사용자가 직접 확인한 오학동 계정을 기존 지출결의·기안의 작성자와 해당 결재 단계에 연결했다. 17:17 KST에는 간편지출 영수증 OCR 연결 migration `20260908081027`을 추가 적용하고 전용 rollback 회귀와 원본 수치 재대조를 통과했다. 계정이 아직 없는 장현제·안동연 단계는 미연결 상태다.
...
## 간편지출 영수증 OCR 운영 연결 — 2026-09-08

간편지출 원본과 기존 `expense-evidence` OCR 작업을 연결하는 관계·처리키·감사 테이블을 추가했다. 지출관리 상세에서 사용내용과 거래처를 수정하고 영수증을 올릴 수 있으며, OCR이 인식한 품목과 거래처는 사용자가 확인한 뒤 저장한다. OCR 금액이 원본 금액과 달라도 지급·예산 원본은 자동 변경하지 않고 차이를 표시한다. 연결된 원본의 내용을 수정하면 기존 통합 거래의 서명 차이로 감지되어 지급·신탁·회계에서 최신 원본 확인 절차를 거친다.

적용 직전 운영 DB 전체 논리 백업을 새로 만들었다. migration은 하나의 transaction으로 적용하고 이력 head를 `20260908081027`로 기록했다. 적용 전후 지출결의 건수·금액, 간편지출 건수·금액, 전표 건수, Storage 메타 건수가 같았고, 운영 DB에서 전용 SQL 회귀를 rollback 방식으로 통과했다. 실제 영수증 파일 업로드는 사용자의 화면 테스트 때 수행한다.

## 운영비 간편지출·대체증빙 정책 — 2026-09-08

승인된 월 예산 안의 소모품비·도서인쇄비·수선비·여비교통비·통신비를 간편지출 허용 목록으로 명시했다. 목록은 관리자 설정에서 바꿀 수 있으며, 목록 밖 지출과 고위험 키워드·한도 초과·예산 초과는 정식 지출결의로 전환한다.

법인카드와 일반 간편지출은 영수증 첨부 전 `증빙 확인대기`로 유지한다. 영수증이 없으면 주문내역·카드 승인내역·계좌이체 확인증·거래명세서·물품 사진 등 대체증빙과 미첨부 사유를 저장하고, 권한 있는 다른 담당자가 승인하거나 보완을 요구한다. 개인카드·개인계좌 이체·현금 선지출 정산에도 같은 검토 상태를 적용하며 신청자는 자신의 대체증빙을 승인할 수 없다. 실제 사용일, 신청일, 승인일, 실제 지급일은 서로 덮어쓰지 않는다.

`20260908113000` 적용 직전 백업은 `.tmp-repos/finance-operational-db-backup-2026-09-08T10-18-48-328Z-870522d5`, 예산 범위 보완 `20260908121500` 적용 직전 백업은 `.tmp-repos/finance-operational-db-backup-2026-09-08T10-24-08-994Z-670dfafe`, 대체증빙 승인자 분리 `20260908123500` 적용 직전 백업은 `.tmp-repos/finance-operational-db-backup-2026-09-08T10-29-34-579Z-b54ee042`에 생성했다. 세 migration을 각각 단일 transaction으로 적용했고 최신 운영 head는 `20260908123500`이다. 보완 후 미정리 원본은 같은 예산항목·같은 귀속월만 간편지출과 개인 정산 승인을 제한하며, 간편지출 대체증빙도 파일 제출자와 승인자가 같으면 승인할 수 없다. 각 적용 전후 지출결의·간편지출·개인정산·전표·Storage 메타 수치가 같았고, 대체증빙 승인·본인승인 차단·중복 처리키·다른 예산 미정리 건 격리 회귀를 운영 DB에서 rollback 방식으로 통과했다.
